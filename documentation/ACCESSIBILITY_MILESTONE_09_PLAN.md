...
### Fine-aim procedural tone extension

After the original alignment cadence passed acceptance, the project owner requested continuous fine-grain accuracy feedback. A temporary F9 experiment rejected `SFX_MAULER_CHARGE` because its audible sample ended and rejected `SFX_LASER_STREAM` because the asset contains its own pitch modulation. A procedural sine prototype with a `1.5` to `3.0` pitch range was accepted as a viable carrier, then integrated into this targeting slice and the F9 binding was removed.

The firing-range adapter does not derive accuracy from projected bounds and does not repeat the collision query. `propFindAimingAtWithHit` optionally returns the hit coordinate already calculated by the existing non-shooting, non-random `FINDPROPCONTEXT_QUERY` path. `lvRender` passes that fixed coordinate into the existing pre-render capture only when the queried prop survives the game's normal final `lookingatprop` filtering. The adapter computes the same Euclidean hit-to-target-center distance used by `frCalculateHit`; its known scoring boundaries are 18, 37, and 56 units. It normalizes continuously over a 75-unit outer scoring radius and submits optional generic `aimquality` plus raw `aimdistance` fields. No weapon spread, firing result, score, damage, or target state is read into the quality value or changed.

The targeting core now enables one centered procedural oscillator only for a visible aimed candidate whose shootability is explicitly `shootable`. Quality zero maps to 660 Hz and quality one maps perceptually to 1320 Hz. It updates every observation while logging bounded 10 Hz snapshots. Aim loss, identity change, facing away, scope loss, feature disable, stage teardown, and shutdown disable the tone. The former accessibility `SFX_0007` hold cadence and direct-sound handle are removed; native game sounds remain untouched. The backend mixes after the normal stereo mix using a fixed staging buffer, preserved phase, per-buffer frequency interpolation, and a 10 ms gain ramp, with no runtime allocation or game sound handle. Runtime correlation of pitch, logged distance, and actual scoring rings remains pending project-owner acceptance testing.
