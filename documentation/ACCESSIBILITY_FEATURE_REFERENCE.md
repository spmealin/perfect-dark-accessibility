...
`Accessibility.IRScannerAudio` defaults to `1` and is subordinate to `Accessibility.Enabled`. While the native IR Scanner is active, an object that receives the visual scanner highlight and whose finite projected model rectangle intersects the current viewport uses the R-Tracker's yellow-object 1450 Hz spatial pattern. Up to ten highlighted objects share the same fixed round-robin slot pool; the game does not permit the R-Tracker and IR Scanner to be active together. The rule is semantic rather than stage-specific, so the IR-highlighted conditional scenery in Skedar Ruins: Battle Shrine receives the same rapid cue while it is on screen.
...
The cue inherits R-Tracker bearing, distance cadence, relative-height pattern, lifecycle suppression, fixed capacity, and allocation-free mixer behavior. A fixed pre-render cache captures highlighted model bounds before PC rendering mutates their matrices; candidates require the current-render flag, a finite projection, and viewport intersection. This deliberately removes the R-Tracker's rear/off-screen modulation from IR highlights, because an IR cue now exists only for something visibly highlighted in the viewport. It adds no separate line-of-sight scan or object name. Logs identify `source=ir_scanner`, `category=infrared_highlight`, projection capacity, and the number of highlighted objects rejected outside the viewport.
...
Armed hostile `OBJTYPE_CHOPPER` aircraft are also part of the generic combat
target set. They use the normal enemy presence cue, manual-aim precision
guidance, and solid hostile lock tone. Eligibility comes from live engine state:
the aircraft must be active, enabled, visible, healthy, armed, neither inactive,
falling, nor dead, and natively targeting the current player. This keeps idle or
scripted scenery aircraft out without a stage or model allowlist.
...
- Interactable objects use one short positioned 880 Hz sine chirp; collectible items use three 35 ms 880 Hz chirps separated by 25 ms; doors use one short positioned 440 Hz chirp through a dedicated voice with a 0.24 mixer level, 50 percent above the former shared chirp lane. The Skedar Ruins bridge-puzzle rock is an authored F5 exception: because it moves when the player walks into it rather than through the normal interaction command, it uses the collectible three-chirp rhythm at the interactable 880 Hz pitch. Door voices retain independent position and volume but take turns in non-overlapping shared-clock windows. Door distance and stereo direction remain anchored to the closed-position center of the complete doorway rather than following an opening leaf toward the frame. The patterns retain distance attenuation and stereo direction while avoiding game/menu sound meanings.
...
A missing floor or a downward change greater than `Accessibility.VirtualCaneDropHeightThreshold`, default 80 units, is a large-drop candidate. Five bounded floor probes refine the last-supported/first-unsupported interval, and at least two unsupported results are required to reject an isolated room-resolution miss. An isolated drop retains the 260 ms steep falling contour at the estimated floor-to-drop transition. When two or more adjacent rays have confirmed an exposed edge with compatible positions, a dedicated 390 Hz warning moves back and forth along their estimated edge points while the normal cane rays remain audible. A known floor below the edge must be at least 120 units lower, or 1.5 times the configured drop threshold, whichever is greater; a confirmed edge with no floor also qualifies. This stronger gate keeps ordinary steps out of the moving warning. The first qualifying ray supplies an immediate drop contour, while subsequent grouped rays give way to the moving cue. The edge is cleared on cane stop, observer change, or each new sweep and rebuilt only from fresh confirmed samples. It takes priority over a wall beyond the edge, while a nearer wall or railing still masks an unreachable drop. The base drop threshold is configurable from 30 through 500 units.
...
on the Attack Ship, the Skedar Ruins bridge-puzzle pressure pad, and the
final-boss spikes in Skedar Ruins. They follow the shared active
...
objective and that objective remains incomplete. The bridge-puzzle pad sounds
while the movable rock is outside the same placement bounds used by the level
script. It stops as soon as the rock holds the bridge open and returns if the
rock is moved away and the bridge can retract. The three shuffled Target
Amplifier pillars deliberately do not use authored mission-marker audio; the
R-Tracker provides their navigation cue, and equipping the Target Amplifier
provides exact alignment feedback. Completed pillars are removed from the
accessible R-Tracker using the level's authoritative placement flags. The three shield consoles
...
requires a stage plus either a setup tag or a fixed setup pad, optional objective gate,
