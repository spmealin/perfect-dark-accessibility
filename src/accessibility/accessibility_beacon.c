...
struct accessibilitybeacondiagnosticfocus {
	struct prop *prop;
	f32 distance;
	f32 dot;
};
...
struct accessibilitybeacondoorslot {
	s32 active;
	s32 restart;
	struct accessibilitybeaconresult result;
};
...
static struct accessibilitybeacondoorslot
		g_AccessibilityBeaconDoorSlots[ACCESSIBILITY_TONE_DOOR_SLOT_COUNT];
...
static void accessibilityBeaconStopDoorChirps(const char *reason)
{
	s32 slot;
	s32 active = 0;

	for (slot = 0; slot < ACCESSIBILITY_TONE_DOOR_SLOT_COUNT; slot++) {
		active += g_AccessibilityBeaconDoorSlots[slot].active != 0;
	}

	accessibilityToneStopDoors();
	memset(g_AccessibilityBeaconDoorSlots, 0,
			sizeof(g_AccessibilityBeaconDoorSlots));

	if (active > 0) {
		accessibilityLogEvent("beacon", "door_chirp_stop",
				"reason=%s active_slots=%d frequency_hz=440 period_ms=750",
				reason ? reason : "unknown", active);
	}
}
...
static void accessibilityBeaconLogTelemetry(const char *reason)
{
	...
	for (i = 0; i < ACCESSIBILITY_TONE_DOOR_SLOT_COUNT; i++) {
		doorslots += g_AccessibilityBeaconDoorSlots[i].active != 0;
	}
	...
	accessibilityLogEvent("beacon", "telemetry",
			"reason=%s tick=%d scans=%llu pulses=%llu memory_available=%d working_set_bytes=%llu working_set_delta=%lld private_bytes=%llu private_delta=%lld snd_states=%d prop_channels_in_use=%d prop_channels_total=%d prop_channels_stopped=%d procedural_chirp_lane=1 door_chirp_slots=%d friendly_drone_slots=%d schedule_targets=%d schedule_cursor=%d next_schedule_tick=%d object_active=%d door_active=%d pickup_active=%d non_hostile_active=%d",
			...
			g_SndNumPlaying, inuse, channels, stopped, doorslots, friendlyslots,
...
static f32 accessibilityBeaconCategoryScanDistance(s32 category)
{
	return category == ACCESSIBILITY_BEACON_CATEGORY_DOOR
...
static s32 accessibilityBeaconScan(s32 detailed)
{
	...
	accessibilityLogEvent("beacon", "scan_start",
			"scan=%llu mode=%s stage=%d player=%d observer_prop=%p observer_propnum=%d observer_remote=%d position=%.3f,%.3f,%.3f heading=%.3f rooms=%d,%d,%d,%d,%d,%d,%d,%d radius=%.1f door_radius=%.1f capacity=%d",
			(unsigned long long)g_AccessibilityBeaconScanCount,
...
static void accessibilityBeaconDeactivateCategory(s32 category, const char *reason)
{
	...
	} else if (category == ACCESSIBILITY_BEACON_CATEGORY_DOOR) {
		accessibilityBeaconStopDoorChirps(reason);
	}
...
static void accessibilityBeaconSuspend(const char *reason)
{
	...
	accessibilityToneStopChirp();
	accessibilityBeaconStopDoorChirps(
			reason ? reason : "scope_suspended");
...
static s32 accessibilityBeaconDoorResultAssigned(s32 resultindex)
{
	s32 slot;
	...
	for (slot = 0; slot < ACCESSIBILITY_TONE_DOOR_SLOT_COUNT; slot++) {
		...
		if (door->active
				&& accessibilityBeaconFindResultIdentity(&door->result)
						== resultindex) {
			return true;
		}
	}
...
static void accessibilityBeaconRefreshDoorChirps(const char *reason)
{
	s32 slot;
	s32 index;
	...
	for (slot = 0; slot < ACCESSIBILITY_TONE_DOOR_SLOT_COUNT; slot++) {
		...
		index = accessibilityBeaconFindResultIdentity(&door->result);
		if (index >= 0) {
			door->result = g_AccessibilityBeaconResults[index];
		} else {
			accessibilityLogEvent("beacon", "door_chirp_release",
					"slot=%d reason=target_removed propnum=%d entity=%p refresh_reason=%s",
					slot, door->result.propnum, door->result.entity,
					reason ? reason : "unknown");
			accessibilityToneSetDoorSlot(slot, false, 0.0f, 0.0f, false);
	...
	for (index = 0; index < g_AccessibilityBeaconResultCount; index++) {
		...
		if (result->category != ACCESSIBILITY_BEACON_CATEGORY_DOOR
				|| accessibilityBeaconDoorResultAssigned(index)) {
			continue;
		}
		...
		for (slot = 0; slot < ACCESSIBILITY_TONE_DOOR_SLOT_COUNT; slot++) {
			...
			if (farthest < 0 || door->result.distance
					> g_AccessibilityBeaconDoorSlots[farthest].result.distance) {
				farthest = slot;
			}
		}
		...
		if (targetslot < 0 && farthest >= 0
			...
			accessibilityLogEvent("beacon", "door_chirp_release",
					...
					targetslot,
					g_AccessibilityBeaconDoorSlots[targetslot].result.propnum,
					g_AccessibilityBeaconDoorSlots[targetslot].result.entity,
					result->propnum, result->entity);
		}

		if (targetslot >= 0) {
			struct accessibilitybeacondoorslot *door
					= &g_AccessibilityBeaconDoorSlots[targetslot];

			door->active = true;
			door->restart = true;
			door->result = *result;
			accessibilityLogEvent("beacon", "door_chirp_assign",
					"slot=%d propnum=%d canonical_propnum=%d entity=%p distance=%.3f reason=%s frequency_hz=440 period_ms=750",
					targetslot, result->propnum, result->canonicalpropnum,
					result->entity, result->distance,
					reason ? reason : "unknown");
		}
	}
}

static void accessibilityBeaconUpdateDoorChirps(void)
{
	s32 slot;
	...
	for (slot = 0; slot < ACCESSIBILITY_TONE_DOOR_SLOT_COUNT; slot++) {
		...
		prop = accessibilityBeaconValidateResult(&door->result, &validreason);
		if (!prop) {
			accessibilityLogEvent("beacon", "door_chirp_release",
					"slot=%d reason=%s propnum=%d entity=%p",
					slot, validreason, door->result.propnum,
					door->result.entity);
			accessibilityToneSetDoorSlot(slot, false, 0.0f, 0.0f, false);
			memset(door, 0, sizeof(*door));
		...
		volume = psCalculateVolumeFromDistance(door->result.distance,
		...
		pan = psCalculatePan(&doorwayposition,
				...
				door->result.distance, false, NULL);
		normalizedvolume = (f32)volume / (f32)AL_VOL_FULL;
		normalizedpan = ((f32)pan - (f32)AL_PAN_CENTER)
				/ (f32)AL_PAN_CENTER;

		accessibilityToneSetDoorSlot(slot, volume > 0,
				normalizedvolume, normalizedpan, door->restart);
		door->restart = false;
	}
}
		...
		const char *reason)
{
	...
	for (category = ACCESSIBILITY_BEACON_CATEGORY_OBJECT;
		...
		if (!g_AccessibilityBeaconCategoryActive[category]
				|| category == ACCESSIBILITY_BEACON_CATEGORY_DOOR
				|| category == ACCESSIBILITY_BEACON_CATEGORY_NON_HOSTILE) {
			continue;
		}
...
static void accessibilityBeaconRefreshActive(void)
{
	...
	accessibilityBeaconRefreshDoorChirps("automatic_refresh");
...
static void accessibilityBeaconRescanActive(const char *reason)
{
	...
	accessibilityBeaconRefreshDoorChirps(reason);
...
void accessibilityBeaconTick(void)
{
	...
	if (g_AccessibilityBeaconCategoryActive[
			ACCESSIBILITY_BEACON_CATEGORY_DOOR]
			&& g_Vars.lvupdate60 > 0) {
		accessibilityBeaconUpdateDoorChirps();
	}
...
void accessibilityBeaconReset(const char *reason, s32 preservecategories)
{
	...
	accessibilityLogEvent("beacon", "reset",
			...
			ACCESSIBILITY_BEACON_SCAN_DISTANCE,
			ACCESSIBILITY_BEACON_DOOR_SCAN_DISTANCE,
			ACCESSIBILITY_BEACON_PULSE_TICKS,
...
static s32 accessibilityBeaconDiagnosticFocusBefore(
		const struct accessibilitybeacondiagnosticfocus *a,
		const struct accessibilitybeacondiagnosticfocus *b)
{
	if (a->dot > b->dot + 0.0001f) {
		return true;
	}
...
static void accessibilityBeaconDumpDiagnosticCandidate(u64 captureid,
		const struct accessibilityobserver *observer,
		const struct accessibilitybeacondiagnosticfocus *focus, s32 index)
{
	...
	const char *finalreason = semanticreason;
	s32 semanticeligible = false;
	s32 finaleligible = false;
	s32 categoryactive = false;
	s32 contexteligible = true;
	...
	if (prop->type == PROPTYPE_DOOR) {
		...
		categoryactive = g_AccessibilityBeaconCategoryActive[
				ACCESSIBILITY_BEACON_CATEGORY_DOOR];
		semanticeligible = accessibilityBeaconDoorEligible(
				prop, &semanticreason);
		finalreason = semanticreason;

		if (!categoryactive) {
			finalreason = "door_category_inactive";
		} else if (semanticeligible) {
			struct prop *canonical = accessibilityBeaconCanonicalDoor(
					prop, &siblingcount);
			...
			if (!candidate) {
				...
				|| (g_Vars.currentplayer
					&& g_Vars.currentplayer->eyespy
					&& g_Vars.currentplayer->eyespy->prop == prop))) {
		semanticeligible = accessibilityBeaconPickupEligible(
				prop, &pickupreason);

		if (semanticeligible) {
			result.category = ACCESSIBILITY_BEACON_CATEGORY_PICKUP;
			result.kind = ACCESSIBILITY_BEACON_KIND_PICKUP;
			semanticreason = pickupreason;
			categoryactive = g_AccessibilityBeaconCategoryActive[
					ACCESSIBILITY_BEACON_CATEGORY_PICKUP];
		} else {
		...
		if (observer->isremote) {
			...
			finalreason = result.category
					== ACCESSIBILITY_BEACON_CATEGORY_PICKUP
					? "pickup_category_inactive"
					: "object_category_inactive";
		}
	} else if (prop->type == PROPTYPE_CHR) {
		result.category = ACCESSIBILITY_BEACON_CATEGORY_NON_HOSTILE;
		result.kind = ACCESSIBILITY_BEACON_KIND_NON_HOSTILE;
		categoryactive = g_AccessibilityBeaconCategoryActive[
				ACCESSIBILITY_BEACON_CATEGORY_NON_HOSTILE];
		semanticeligible = accessibilityBeaconNonHostileEligible(
				prop, observer->isremote, &semanticreason);
		finalreason = semanticreason;

		if (!categoryactive) {
			finalreason = "non_hostile_category_inactive";
		}
	}

	if (candidate && (candidate->type == PROPTYPE_OBJ
			|| candidate->type == PROPTYPE_WEAPON
			|| candidate->type == PROPTYPE_DOOR)) {
		obj = candidate->obj;
	}
	...
	if (semanticeligible && categoryactive && contexteligible
		...
		withinrange = distance * distance
				<= ACCESSIBILITY_BEACON_SCAN_DISTANCE_SQ;
		roomsrelated = accessibilityBeaconRoomsRelated(
				observer->prop->rooms, candidate->rooms);
	...
	accessibilityLogEvent("incident", "beacon_candidate",
			...
			categoryactive, contexteligible,
			accessibilityBeaconKindName(result.kind), (void *)prop,
			accessibilityBeaconPropNum(prop), prop->type, (void *)candidate,
...
static void accessibilityBeaconDumpDiagnosticFocus(u64 captureid)
{
	struct accessibilityobserver observer;
	struct accessibilitybeacondiagnosticfocus
			focus[ACCESSIBILITY_BEACON_DIAGNOSTIC_FOCUS_CAPACITY];
	struct prop *prop;
	s32 count = 0;
	s32 supported = 0;
	s32 infront = 0;
	...
	if (!accessibilityObserverGet(&observer)) {
		accessibilityLogEvent("incident", "beacon_candidate_summary",
	...
	while (prop && prop != g_Vars.pausedprops
		...
		f32 distance;
		f32 dot;
		struct accessibilitybeacondiagnosticfocus item;
		s32 position;

		if (prop == observer.prop
				|| !accessibilityBeaconDiagnosticFocusSupported(prop)) {
			prop = prop->next;
			continue;
		}

		supported++;
		dx = prop->pos.x - observer.camera.x;
		dy = prop->pos.y - observer.camera.y;
		dz = prop->pos.z - observer.camera.z;
		distsq = dx * dx + dy * dy + dz * dz;

		if (distsq <= 0.0001f
				|| distsq > ACCESSIBILITY_BEACON_SCAN_DISTANCE_SQ) {
			prop = prop->next;
			continue;
		}

		distance = sqrtf(distsq);
		dot = (dx * observer.look.x
				+ dy * observer.look.y
				+ dz * observer.look.z) / distance;
		...
		if (dot < ACCESSIBILITY_BEACON_DIAGNOSTIC_FOCUS_DOT) {
			prop = prop->next;
			continue;
		}
		...
		if (count < ACCESSIBILITY_BEACON_DIAGNOSTIC_FOCUS_CAPACITY) {
			focus[count++] = item;
		} else if (accessibilityBeaconDiagnosticFocusBefore(
				&item, &focus[count - 1])) {
			focus[count - 1] = item;
		} else {
			prop = prop->next;
		...
		while (position > 0
				&& accessibilityBeaconDiagnosticFocusBefore(
					&focus[position], &focus[position - 1])) {
			struct accessibilitybeacondiagnosticfocus temp
					= focus[position - 1];

			focus[position - 1] = focus[position];
			focus[position] = temp;
			position--;
		}
	...
	for (i = 0; i < count; i++) {
		accessibilityBeaconDumpDiagnosticCandidate(
				captureid, &observer, &focus[i], i);
	}

	accessibilityLogEvent("incident", "beacon_candidate_summary",
			"capture=%llu result=complete traversed=%d supported=%d in_front=%d in_focus_cone=%d logged=%d capacity=%d truncated=%d minimum_dot=%.3f",
			(unsigned long long)captureid, traversed, supported,
			infront, incone, count,
			ACCESSIBILITY_BEACON_DIAGNOSTIC_FOCUS_CAPACITY,
			incone > count, ACCESSIBILITY_BEACON_DIAGNOSTIC_FOCUS_DOT);
}
...
void accessibilityBeaconDumpDiagnostics(u64 captureid)
{
	...
	accessibilityBeaconDumpDiagnosticFocus(captureid);
}
