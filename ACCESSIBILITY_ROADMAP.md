...
Implementation status: a general single-player virtual-cane slice is implemented as an orientation experiment, but Milestone 11 remains open. F4 cycles off, a two-second seven-angle sweep, and a one-second seven-angle sweep, with distinct mode earcons. Each live player-sized movement-cylinder probe reports the nearest background/object/door/path-blocking collision while excluding characters. Spatial pan and volume communicate impact position; distance changes the chirp pitch; longer rising/falling sweeps report detected upward/downward terrain transitions. A rising transition uses the up contour only when the live player envelope can traverse it; a failed traversal reports the rise with an ordinary barrier chirp. For either direction, a non-drop terrain transition with less than two live player diameters of runway before a detected barrier cedes to that barrier, preventing a shallow dead-end pocket from sounding like a usable route. A confirmed large drop produces a steeper 260 ms falling contour positioned at the refined floor edge and takes priority over a farther wall. A conditional full-squat collision sweep converts a standing obstruction into a two-chirp crouch-passage cue when Joanna could traverse it while squatting; terrain within one player radius no longer masks that test. A 90 ms falling-octave variation identifies a healthy, mortal `OBJFLAG_PATHBLOCKER`, and the targeting lane accepts the same obstruction only when the exact attack ray hits it with a compatible attack. Reach, drop threshold, and master volume are configurable. The base collision, pitch, terrain behaviors, and captured DataDyne route-blocking pane received project-owner blind-user acceptance on 2026-07-28; drop-edge, revised crouch-passage behavior, and terrain-runway prioritization await runtime acceptance. Broader breakable-obstruction coverage, route guidance, named landmarks, deviation/recovery semantics, controller/settings UI, and independent-user validation remain pending.
