...
- **Status:** A campaign-wide setup audit now maps every confirmed point-target special-device contract, including Uplink/Reprogrammer terminals and vehicles, ECM/Remote Mine/Tracer Bug/Comms Rider/Target Amplifier placement surfaces, the Door Decoder keypad, Infiltration explosives terminal, and Escape Auto-Surgeon hoverbed. Static integration is complete; per-mission, difficulty, and co-op blind-user acceptance remains pending. The authoritative coverage and exclusion matrix is `documentation/ACCESSIBILITY_SPECIAL_DEVICE_TARGET_AUDIT.md`.
...
- **Behavior:** While a registered item is equipped, pointing at an exact stage/tag/type target produces a fixed 660 Hz centered tone. The registry admits all confirmed targets listed in `documentation/ACCESSIBILITY_SPECIAL_DEVICE_TARGET_AUDIT.md`, supports multiple destinations for one item, and treats Chicago's upper/lower mine surfaces as door props rather than generic objects. A separate device profile suppresses positioned presence audio. Native range, use, throw, objective, inventory, and difficulty rules remain authoritative; the cue identifies a destination surface but does not predict impact or promise success.
...
- **Risks:** These validity rules exist as setup-script tags rather than a general runtime capability API. Setup changes and alternate difficulty/co-op state can invalidate a row, so the audit matrix and runtime diagnostics must be checked during mission playthroughs. The raw query can identify a surface but cannot establish interaction range or simulate a thrown device trajectory.
...
- **Explicit non-goals:** No aim automation, through-wall disclosure outside a native X-Ray/FarSight presentation, spoken identity, friendly/neutral cue vocabulary, head-quality pitch mapping, multiplayer policy, or automatic support for vehicles and other non-character threats.

Engineering extension: while the FarSight XR-20's native X-Ray sight is active, a combat candidate rendered within the native eraser volume can substitute that render evidence for ordinary physical line of sight. This applies to both manual-depth primary aiming and automatic Target Locator aiming. Eligible on-screen hostiles use the established enemy presence voice, and the unchanged native FarSight query ray drives the existing locked-on tone when the crosshairs intersect one. In the CI firing range, native FarSight-visible range targets use the configured scoped-distance presence curve rather than the ordinary 1,400-unit range cutoff, and an exact native FarSight hit permits alignment even though this sight omits the ordinary target-indicator widget. The standalone X-Ray Scanner remains the only source that broadens F5-F8 semantic scanners; FarSight visibility is confined to combat targeting. This extension is implemented pending runtime acceptance.
...
- **Behavior:** F7 independently toggles up to three automatically refreshed friendly, neutral, or native blue-sight protected character beacons in any single-player gameplay stage. Each retained person owns a continuous positioned drone combining a 440 Hz fundamental and quieter 550 Hz major third. Protected nonlethal targets also admit the valid-aim alignment tone but never receive hostile enemy-presence cadence. Existing relationship, life/action, hidden, untargetable, cloak/IR, room, range, and semantic visual line-of-sight state govern eligibility; engine-marked transparent props do not hide an otherwise visible person.
- **Door visibility refinement:** F6 door candidates must have finite projected model bounds intersecting the active camera viewport at scan and playback time; the engine's coarse current-screen flag alone is insufficient. If any part of any native sibling leaf is on-screen, one canonical doorway cue may sound; if every leaf is outside the viewport, it must remain silent. Native sibling leaves retain one canonical doorway identity and at most one viewport-intersecting positioned source, so paired leaves cannot consume multiple result or schedule slots.
- **Acceptance:** Test friendly and neutral people, scripted allegiance changes, nearby hostiles, death/knockout, cloak with and without IR perception, occlusion, movement, more than three candidates, stage transitions, menus, pause, cutscenes, death, and repeated sessions. Confirm F7 changes only this category, all three retained people can remain spatially trackable together, and the chord is distinct from pickup chirps and player markers.
- **Evidence:** Correlate `beacon/candidate`, `scan_result`, `friendly_drone_assign`, `friendly_drone_release`, `friendly_drone_stop`, and `category_state` records with team, action, visibility, identity, position, distance, pan, and fixed-slot output.
- **Risks:** A neutral relationship can change through scripts after a refresh; multiple continuous voices require masking and dense-mix tests. A midpoint line-of-sight ray may exclude a partly visible person. The three-target cap prioritizes stable nearby identities rather than exhaustive awareness.
...
Implementation status: a general single-player virtual-cane slice and cardinal-compass prototype are implemented as orientation experiments, but Milestone 11 remains open. Plain F4 cycles off, a two-second nine-angle sweep, and a one-second nine-angle sweep, with distinct mode earcons. Each live player-sized movement-cylinder probe reports the nearest background/object/door/path-blocking collision while excluding characters. Spatial pan and volume communicate impact position; distance changes the chirp pitch; longer rising/falling sweeps report detected upward/downward terrain transitions. A rising transition uses the up contour only when the live player envelope can traverse it; a failed traversal reports the rise with an ordinary barrier chirp. For either direction, a non-drop terrain transition with less than two live player diameters of runway before a detected barrier cedes to that barrier, preventing a shallow dead-end pocket from sounding like a usable route. A confirmed large drop produces a steeper 260 ms falling contour positioned at the refined floor edge and takes priority over a farther wall unless the wall lies within one player radius beyond that edge. A conditional full-squat collision sweep converts a standing obstruction into a two-chirp crouch-passage cue when Joanna could traverse it while squatting; terrain within one player radius no longer masks that test. A 90 ms falling-octave variation identifies a healthy, mortal `OBJFLAG_PATHBLOCKER`, and the targeting lane accepts the same obstruction only when the exact attack ray hits it with a compatible attack. Reach, drop threshold, and master volume are configurable. Shift+F4 independently cycles an Off, spoken-plus-sonified, and sonification-only compass; exact North/East/South/West crossings produce one through four centered clicks clockwise from North, with optional matching speech and jitter hysteresis. The base collision, pitch, terrain behaviors, and captured DataDyne route-blocking pane received project-owner blind-user acceptance on 2026-07-28; drop-edge, revised crouch-passage behavior, terrain-runway prioritization, and the compass await runtime acceptance. Broader breakable-obstruction coverage, route guidance, named landmarks, deviation/recovery semantics, controller/settings UI, and independent-user validation remain pending.
