...
- **Status:** Engineering implementation added for the Data Uplink, ECM Mine, and Door Decoder exercises; runtime and blind-user acceptance are pending.
...
- **Status:** Engineering implementation builds; Holo Training 4 runtime and blind-user acceptance are pending.
- **Goal:** Extend the generic targeting core from firing-range props to ordinary hostile characters without using stage, holo-training, or script-specific identifiers.
- **Systems:** Use the bounded onscreen-prop list, rendered model bounds, existing team/friendly classification, character life/action/visibility/targetability state, shooting-blocker line of sight, and the finalized attack-query prop. Feed admitted characters into ten fixed procedural presence voices and the direct-aim alignment lane. Keep a separate combat profile so changing between range and combat clears stale identities and audio.
- **Initial behavior:** Active, enabled, combat-capable, rendered, visible hostile characters receive positioned 440 Hz cues in independent staggered slots for up to ten enemies. The combat-specific audible envelope is full through 600 world units, fades through 3,500, and ends at 4,000, with a 0.14 mixer gain. Relative to the semantic unarmed punch range `X`, cadence is 180 ms every 750 ms beyond `5X`, ramps to 50 ms every 200 ms at `X`, and becomes a constant tone inside `X`. Material period reductions trigger the next chirp immediately. Constant-mode entry remains `X`, with an exit-only `1.1X` hysteresis threshold to suppress collision-boundary flicker. Directly pointing at an admitted hostile produces a separate fixed 660 Hz centered alignment tone, including while unarmed. Friendly, neutral, dying, dead, falling-knocked-out, knocked-out, hidden, untargetable, imperceptibly cloaked, offscreen, and occluded characters do not produce positive feedback.
