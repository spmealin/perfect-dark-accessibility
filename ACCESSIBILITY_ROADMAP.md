...
Implementation status: a general single-player virtual-cane slice is implemented as an orientation experiment, but Milestone 11 remains open. F4 cycles off, a two-second seven-angle sweep, and a one-second seven-angle sweep, with distinct mode earcons. Each live player-sized movement-cylinder probe reports the nearest background/object/door/path-blocking collision while excluding characters and drop-offs. Spatial pan and volume communicate impact position; distance changes the chirp pitch; longer rising/falling sweeps report detected upward/downward terrain transitions. A 90 ms falling-octave variation identifies a healthy, mortal `OBJFLAG_PATHBLOCKER`, and the targeting lane accepts the same obstruction only when the exact attack ray hits it with a compatible attack. Reach and master volume are configurable. The base collision, pitch, terrain behaviors, and captured DataDyne route-blocking pane received project-owner blind-user acceptance on 2026-07-28. Broader breakable-obstruction coverage, route guidance, named landmarks, deviation/recovery semantics, controller/settings UI, and independent-user validation remain pending.
