...
The generic combat object source admits armed hostile `OBJTYPE_CHOPPER`
aircraft alongside hostile characters, autoguns, and security cameras. Its
semantic gate uses native activation, invisibility, health, weapon-armed,
attack-mode, death, and current-target state; inactive scenery, falling or
destroyed aircraft, and aircraft targeting somebody other than the current
player are excluded. Admitted aircraft reuse the fixed combat projection,
model-surface visibility, precision-guidance, slot, and exact raw-query alignment
paths. No setup tag, stage number, mission objective, or model number participates.
...
Authored navigation landmarks may resolve either a tagged prop or a fixed setup pad. Tagged props use bounded model-surface visibility; pad destinations use their setup position and room with the same semantic line-of-sight policy. The Skedar Ruins bridge puzzle uses the latter form for `PAD_SHO_00D4` and mirrors the level script's tag `0x4c` placement box every tick, so the pad marker stops when the movable rock holds the bridge open and returns if the rock moves away. The rock remains an F5 scanner exception rather than a landmark: tag `0x4c` receives the three-pulse pickup rhythm at the normal 880 Hz interactable pitch while retaining ordinary viewport, range, and line-of-sight gates.
...
The IR Scanner continues to reuse the R-Tracker's ten voices for its viewport-bound special highlights. `objIsHighlightedByInfrared` is shared with `objRender`, so IR admits only conditional-scenery and explicit-infrared objects that receive the special white highlight. A fixed 64-entry cache captures only those objects at the safe pre-render boundary and records their finite projected model rectangles. The tracker requires the current-render flag and viewport intersection; the engine's broader previous-screen flag is not sufficient. This applies without a stage allowlist, including both explicitly infrared and conditional-scenery weak points in Skedar Ruins. X-Ray does not own a generic audio lane because its renderer recolors every nearby prop without semantic distinction. Instead, `accessibilityVisibilityIsXrayExposed` is a shared alternate-visibility predicate used by the beacon and combat-target adapters. It combines the uninhibited X-Ray Scanner device bit, native vision mode, `PROPFLAG_ONANYSCREENPREVTICK`, and `objGetXrayHighlightDistance`; qualifying props retain their normal semantic category and sound while bypassing only ordinary room-neighbour and physical line-of-sight gates. Existing actionability, collectability, relationship, health, viewport, range, capacity, and lifecycle policies remain authoritative. The preceding-frame flag introduces one frame of deliberate latency while avoiding another projection pass or off-screen disclosure, and device-bit gating excludes the Farsight's separate X-Ray vision mode.
...
Accessibility.InterruptedTargetingVolume=0.12
...
The first structured-query migration separates engine observation from a
...
legacy barrier fallback remains audible. A separate fixed-storage
`accessibility_cane_path` evaluator and native query
...
and misclassified a later wall as a drop. A connected crouch opening now
requires two player radii of verified continuation beyond the first lower
clearance and is spatialized one radius inside the opening. A crouched route
that stops at a wall must additionally recover standing clearance and sustain
it for two player radii; otherwise it is a proven recess and resolves as a
barrier. A crouched route that remains open through the complete query range
does not require a visible exit. Short or uncertain crouch paths retain the
established decision, and native ladder geometry keeps priority. The walking
adapter supplements direct collision flags with one
player-sized native ladder-overlap query at barrier contact, including the
engine's secondary adjacent-room check, so a thin climbable polygon embedded
in an ordinary wall remains observable. The value-only result layer accepts
that overlap only when its native face normal is predominantly horizontal;
floor- and ceiling-facing flagged geometry cannot supply native climbing's
horizontal face response and remains an ordinary barrier. This is a reusable
semantic invariant with platform-independent fixtures, not a stage, model, or
coordinate exception. Vehicle, remote-observer, unsupported, and uncertain results retain
legacy policy. The adapter
...
The migration keeps four explicit boundaries to prevent the structured cane
from becoming another exception chain. The engine adapter acquires bounded raw
facts: collision, floors, clearances, native flags, normals, and query status.
Value-only validators convert those facts into reusable evidence such as a
connected drop, sustained grade, traversable crouch opening, or climbable
ladder face. The result policy resolves competing evidence to one cue and one
reason set. The audio layer renders that decision without inspecting geometry.
New level observations should first become a fixture for one of these generic
validators; a stage number, coordinate, room, prop, or model branch requires a
separate authored-landmark design rather than entry into cane policy.

The virtual-cane core runs once after `lvTick` on the main game thread. It accepts F4 in ordinary single-player walking, grabbed-object, or hoverbike movement, cycles a constructor-registered `0..2` mode, and schedules the nonuniform nine-angle fan at -80, -45, -25, -10, 0, +10, +25, +45, and +80 degrees without resetting for movement. Walking/grab/CamSpy fans remain camera-relative. A mounted fan is centered on current bike velocity, falling back to bike heading at negligible speed, while playback remains camera-relative. Moving a grabbed crate retains the ordinary collision sweep. Each grab query temporarily disables exactly the current player's native `grabbedprop` perimeter; each vehicle query instead disables rider and mounted-bike perimeters and uses the native bike cylinder. Walking-only terrain traversal, crouch-passage, and ladder classifiers stay disabled until normal walking resumes. The slow and fast schedules use region-correct logical ticks, include an end pause, skip overdue work after a hitch, and enforce no more than one collision query per tick.
...
Ten fixed procedural-mixer slots own nine cane-angle chirps and one context slot used by the hoverbike blocked-movement cue. Atomic sequences transfer start/end frequency, duration, pattern, configured master gain, distance attenuation, and pan to audio-owned phase/envelope storage; stop clears all slots. Every ordinary barrier, including a breakable path blocker, uses the same steady distance-pitched 35 ms wall chirp; breakability remains the responsibility of the separate exact-aim targeting policy and does not change navigation audio. A selected connected terrain result is reduced by the value-only path layer to at most ten fixed distance/elevation knots, with an explicit floor-knot count distinguishing the optional terminal wall. The floor knots cover the whole observed path, begin at the player's floor, and preserve the end; a final level run is explicitly retained when uniform reduction would otherwise hide a landing. The publication layer maps absolute elevation relative to that starting floor to a bounded pitch and independently applies the configured attenuation curve to every knot's actual distance. The mixer performs continuous logarithmic frequency interpolation and linear gain interpolation across the supplied floor knots, with no geometry interpretation and no gap or phase reset between them. A separately observed impassable barrier, including one beyond terrain-query reach, reserves the final knot and follows a 5 ms gap with the ordinary distance-pitched wall carrier plus a second harmonic. This uses only fixed atomic storage and represents compound evidence such as `flat, descending, lower flat, wall` without promoting it to a route-quality conclusion. Unavailable or uncertain connected results retain the 140 ms logarithmic terrain-contour fallback. Filling the fixed 32-node path capacity is instead completed connected evidence: flat capacity traces suppress legacy terrain and terrain-bearing capacity traces publish their connected contour. Broad and uncertain terrain decisions are deferred until all rays finish; each contiguous same-direction group retains at most its most-central non-structured contour, while structured evidence replaces the whole group with boundary guidance. This prevents fallback and open-terrain decisions from recreating the obsolete multi-ray elevation sweep. A movement collision carrying `GEOFLAG_LADDER` or `GEOFLAG_LADDER_PLAYERONLY` uses a 220 ms continuous logarithmic rise from the terrain-up starting carrier to twice the ordinary distance pitch, preserving attenuation and the collision point's pan while distinguishing engine-climbable geometry without stage-specific data. Front-loaded floor samples look for support before the nearest movement collision. A missing floor or downward change beyond the configurable 80-unit default brackets a large drop; five bounded refinements require at least two unsafe results and position a 260 ms steep falling contour at the estimated supported/unsupported transition. The nearer edge ordinarily supersedes a wall beyond it, but a wall or railing before the edge remains authoritative. A blocker less than one live player collision radius beyond a refined edge also supersedes it because the intervening gap cannot accept the player's collision envelope; this prevents lower floor behind or beneath a wall from being announced as a reachable ledge. Vehicle reach is the larger of configured reach or one second of current travel plus radius, capped at three times configured reach; vehicle terrain probes cap at 1,200 units and retain only large-drop results. A local walking observer whose current-height cylinder is blocked receives one conditional full-squat-height sweep just beyond the collision. Terrain up to one player radius before the barrier does not suppress this test; if the squat sweep clears, the co-located crouch passage supersedes ordinary terrain. Its 135 ms fixed mixer pattern contains two 55 ms chirps separated by 25 ms, with the second carrier 1.5 times lower than the distance-pitched first carrier. Large drops, CamSpy, hoverbikes, and an already fully squatted envelope skip this classification. A rejected native bike movement attempt emits a camera-relative 220-to-140 Hz, 90 ms cue from the context slot no more than twice per second. All use the bounded `Accessibility.VirtualCaneVolume` master gain, which defaults to 0.184. The F4 command also publishes the selected mode through the centered toggle-confirmation lane: Slow rises in two beeps, Fast rises with two high beeps after the base, and Off falls in two. The path allocates no native game sound channels or dynamic memory. One preallocated text buffer aggregates all nine sample records and uses the logger's preformatted event API, avoiding periodic formatting allocation; the logger batches ordinary disk flushes. Advanced diagnostics expose active profile, vehicle speed/reach/self-exclusion, floor-query/refinement records, drop/barrier gap suppression, current surface plane and grade, per-ray continuation decisions, contour floor/total counts and per-knot type/distance/elevation/frequency/gain, terrain/crouch/ladder classification, tone pattern, standing and crouch collision results, collision timing/counters, and mixer request/active masks.
...
shield consoles, the Skedar Ruins bridge-puzzle pressure pad, and the final-boss spikes.
Registry identity follows the engine's current tag-to-object mapping rather than
asking an object for one tag. Pad entries instead resolve the setup position and
room directly and may use a separate tagged object's live state as their completion gate.
...
| `port/src/audio.c` | Mix procedural accessibility voices into each completed stereo buffer before SDL queueing | Centered targeting tone, firing-range presence pulse, threat-detector new-contact sweep, single/patterned beacon and cane chirps, compass clicks, continuous friendly-character chords, marker sweeps/identities, toggle, stance, and weapon-function patterns, positioned environmental-hazard tone state, hostile/security-camera combat cues, serialized R-Tracker markers, serialized Combat Simulator radar events, and the King of the Hill guide | Clean responsive carriers cannot be made from game samples with finite duration or baked-in modulation | Independent fixed voices share one staging buffer: centered fine aim, one harmonic firing-range round-robin lane, one serialized rising threat-contact lane, one/three-pulse object and pickup beacon patterns, three repeating door-chirp slots, three two-oscillator friendly drone slots, a one/two/three-beep toggle, cane-mode, and stance lane, ten cane slots (nine ray voices plus one shared context voice), one separate moving drop-edge lane, one dedicated one-to-four-click compass lane, four two-oscillator marker slots with one serialized identity lane, one dedicated hill dual-sweep/identity lane whose chirp cadence reports native scoring progress, a one/two-beep weapon-function lane, continuous hazards, ten combat slots with fixed or swept frequency, ten stable R-Tracker contact slots feeding one round-robin lane, and one Combat Simulator radar lane; none allocate at runtime |
...
| `src/game/activemenutick.c` | Observe the settled active-menu screen and highlighted slot once after all sampled input is processed | Primary-player active-menu mode, screen index, slot index, and localized center/focus labels from `amGetSlotDetails` | Weapon, function, Perfect Buddies, and Orders are gameplay radials rather than normal `struct menu` dialogs; observing after input avoids duplicate speech from intermediate controller samples | Every radial screen announces its title on entry and then only changed focus labels; non-primary local-player speech remains suppressed |
...
| `src/game/lv.c` | Capture projected target bounds, scoped character collision-region anchors and bounded geometry validation, deliberately interactable-object and IR-highlight bounds, onscreen hostile-character props, and the existing query-ray hit coordinate/body part before prop rendering; observe after player sight/HUD rendering; expose read-only multiplayer limit getters | Finite projected bounds, hit-region centers and validated surface guidance, final filtered `lookingatprop`, exact query hit and model hit part, native sight/X-Ray state, player viewport, rendered interactable/IR-highlight/character membership, and time/player/team score limits | PC prop rendering converts float model matrices in place before sight/HUD state is final, so post-render accessibility ticks cannot safely project terminals, IR-highlighted scenery, or sniper anchors and one targeting hook cannot obtain both targeting states; status queries must not duplicate private limit globals | Adjacent bounded beacon, tracker, and targeting capture calls plus the later targeting observation support viewport-true scanners, IR highlights, firing-range, generic hostile-character, and FarSight X-Ray combat profiles; scoped guidance retains five fixed collision-region anchors and validates at most one nearby target with 15 allocation-free geometry queries. Query results survive final semantic filtering. Three pure getters supply the F1 status adapter with authoritative limits |
| `src/game/bondgun.c` | Mark forward/back weapon-cycle requests and observe settled weapon/function state after gameplay weapon processing | Requested weapon, player, stage, equipped weapon, both hands' final in-use state, final `bgunIsUsingSecondaryFunction()` value, and the localized function label when the native Show Gun Function option makes it visible | Speech must follow a successful semantic switch rather than input alone; dual wielding is visually apparent only after the left hand enters use, while the function label is rendered directly rather than admitted to the HUD-message queue | Two request markers reuse the existing cycle functions; one end-of-tick observation resolves pending weapon speech, announces newly settled dual wielding, and publishes visible function-label changes, while initial/stage state establishes a silent baseline |
