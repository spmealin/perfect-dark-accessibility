...
For a local walking observer, one read-only native floor query per sweep captures the current polygon normal and derives its grade along the camera-forward vector. A 3.5-percent enter and 2.5-percent exit threshold provide hysteresis. Each ray compares its four existing floor samples with the height predicted by that same plane; only a match within the configured terrain-height tolerance can classify a safe continuation. Upward continuations must also pass the live-stance traversal checks. A safe continuation cedes to a farther wall or silence, while step-flagged geometry, plane deviations, starts, ends, stairs, blocked rises, and drops keep the established ray semantics. Once all nine ray slots finish, one centered 140 ms grade contour plays in the existing end pause through the context slot. It communicates current ascent/descent without masking spatial geometry ahead. The classifier adds no allocation, adds one floor query per complete sweep rather than per ray, and remains disabled for CamSpy, grabbed-object, and mounted movement. Aggregate diagnostics record the surface room/normal/ground delta, forward and ray grade, maximum plane residual, safety/suppression decision, and grade-context emission.
