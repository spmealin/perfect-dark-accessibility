...
| `src/game/bondbike.c` | Publish the settled result of each native hoverbike movement attempt to the bounded incident recorder | Requested displacement/turn, collision result, and immediate obstacle identity | Shift+F2 polling after the tick cannot recover the collision subsystem's transient obstacle result or distinguish a blocked command from an idle vehicle | Implemented as one allocation-free diagnostic hook after native push/retry handling; no movement state or collision decision is changed |
