...
A third, alignment-only device profile handles confirmed special-item target contracts. A fixed registry carries stage, weapon, target tag, and scope for each contract, permits multiple rows for one item, and maps the current two CI exercise rules to the exact object tags used by their AI success scripts (`WEAPON_DATAUPLINK` to `0x30`, `WEAPON_ECMMINE` to `0x32`). Stage-scoped mission rules and additional targets can therefore extend the adapter without changing its core or audio policy. It admits a registered object only while its scope is active, the matching device is equipped, and the prop remains active, enabled, visible, and not deleting or gone. It compares against the raw same-frame attack-query prop because ordinary `lookingatprop` filtering intentionally discards the Uplink's `MODEL_GOODPC`. The core suppresses both round-robin and combat presence for this profile, leaving only the centered 660 Hz alignment lane. This is a semantic registry, not a model/interactable heuristic; devices without a target-selection action receive no fabricated target, and ECM feedback identifies the script-authorized hub but does not predict the projectile trajectory or promise a valid landing.
...
A scanner is a separate user-enabled query over nearby eligible props; it must not reuse render visibility as its entire semantic model or reveal hidden mission information. Milestone 5 first proves this boundary in Carrington Institute training with interactable objects, collectible items, and canonicalized doors. Each enabled category refreshes a bounded snapshot twice per second and retains up to three nearby targets using a 150-unit membership margin; interactables, doors, and pickups are independently controlled by F5, F6, and F8. Doors and interactables require the established background-only line-of-sight ray. Pickups use the collection path's semantic object/collectability flags and door-plus-background sight blockers, excluding invisible, unavailable, or temporarily reserved items. A global round-robin scheduler interleaves all enabled categories and retriggers one procedural chirp voice, guaranteeing that two beacons never start together. Doors use one 440 Hz chirp, interactable objects use one 880 Hz chirp, and pickups use three quick 880 Hz chirps. The core derives normalized distance volume and stereo pan from the established prop-sound spatial calculations before publishing the fixed-size request to the audio mixer. Milestone 10 generalizes categories, names, and query output only after that evidence exists.
...
| `port/src/audio.c` | Mix procedural accessibility voices into each completed stereo buffer before SDL queueing | Centered targeting tone, single/patterned beacon and cane chirps, weapon-function pattern, and positioned environmental-hazard tone state | Clean responsive carriers cannot be made from game samples with finite duration or baked-in modulation | Independent fixed voices share one staging buffer: centered fine aim, one/three-pulse beacon patterns, seven cane slots, a one/two-beep weapon-function lane, continuous hazards, and combat slots; none allocate at runtime |
...
| `port/include/input.h` | Use provisional context-sensitive PC F4/F5/F6/F8 accessibility keys and expose both Alt modifier bits | Development-only action identifiers | Gameplay uses F4 for virtual-cane mode and F5/F6/F8 for CI interactable/door/pickup beacons; menus retain their F5/F6 contexts; Alt+F4 must not change cane state | Implemented through the pickup-beacon extension; replacement by Milestone 6 actions/settings remains required |
