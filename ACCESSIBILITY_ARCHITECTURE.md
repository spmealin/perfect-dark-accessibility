...
| `port/src/audio.c` | Mix procedural accessibility voices into each completed stereo buffer before SDL queueing | Centered targeting tone, short beacon chirp, and positioned environmental-hazard tone state | Clean responsive carriers cannot be made from game samples with finite duration or baked-in modulation | Three independent voices share one fixed staging buffer: centered fine aim, 100 ms beacon chirps, and smoothly panned continuous hazards; none allocate at runtime |
