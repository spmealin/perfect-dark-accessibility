...
The virtual-cane core runs once after `lvTick` on the main game thread. It accepts F4 in ordinary single-player walking or grabbed-object movement, cycles a constructor-registered `0..2` mode, and schedules seven live camera-relative angles without resetting for movement. Moving a grabbed crate remains player-controlled gameplay and retains the ordinary collision sweep. Each query temporarily disables exactly the current player's native `grabbedprop` perimeter, following the grab-movement collision pattern, then restores it after barrier, terrain, and crouch sampling; this prevents the carried object from masking the route without excluding other props. Walking-only terrain traversal and crouch-passage classifiers stay disabled until normal walking resumes. The slow and fast schedules use region-correct logical ticks, include an end pause, skip overdue work after a hitch, and enforce no more than one collision query per tick.
