...
The same adapter supplies a separate generic combat profile from the engine's bounded onscreen-prop list. It admits active, enabled, combat-capable hostile character/player props whose rendered model bounds intersect the viewport and whose body midpoint has shooting-blocker line of sight from the camera. Existing team comparison, friendly classification, cloak/IR, hidden, untargetable, dying/dead, and knockout action state provide semantic filtering; no stage, holo-training, or character-script identifier participates. Characters use the finalized native `lookingatprop` selection. Native standard auto-aim scans only character props, so an admitted autogun uses the separately retained, same-frame non-random query-ray prop for direct alignment. The raw hit can retain an actually aimed, partially visible turret when a midpoint ray alone would reject it, but it never changes weapon aim or bypasses the autogun semantic filters.
