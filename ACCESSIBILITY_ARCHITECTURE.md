...
Targeting uses a two-phase observation. It captures fixed-size projected bounds immediately after aim and tracked-prop calculation, before PC prop rendering converts model matrices in place, then consumes and clears that cache after sight/HUD rendering has finalized native alignment state. The firing-range source admits active, undestroyed `MODEL_TARGET` props; the combat source admits ordinary onscreen hostile character props, hostile `OBJTYPE_AUTOGUN` props, and active `OBJTYPE_CCTV` security cameras. Autoguns reuse native health, deactivation, ammunition, malfunction, and target-team state, so disabled/destroyed/non-threatening turrets and the player's deployed Laptop Gun are excluded without stage identifiers. Cameras reuse native health, deactivation, and camera-disabled state. All require successful finite projection and viewport intersection. Characters match the finalized native target, while autoguns and cameras use the retained non-random attack-query hit for direct crosshair alignment. Ordinary combat naming, other non-character threats, multiplayer output, and special-sight behavior still need investigation.
