...
Sniper precision guidance is a presentation mode over that same eligible combat set, not a second targeting test. The adapter activates it only for the equipped Sniper Rifle while the live FOV is narrowed. During the existing pre-render projection capture it walks the character model's current node tree, projects collision-box centers through their live matrices, excludes gun/hat boxes, and retains at most one nearest-crosshair anchor in each of five fixed semantic groups: head, torso, arm, lower body, and other. The visibility probe that admitted the character supplies a bounded regional preference; the native `chrCalculateAutoAim` position is used only when no model collision anchor is available. Core policy chooses the nearest screen-space target with a fixed identity switch margin, then overrides only that target's existing combat-slot cadence and pan using the selected anchor's normalized error. Its two-part carrier encodes vertical correction while scoped horizontal error controls stereo position. No new collision ray, allocation, voice, or gameplay mutation is introduced. Exact lock remains wholly dependent on the established native/raw aim identity, so neither an anchor nor projected bounds can claim that a shot will hit.
...
| `src/game/lv.c` | Capture projected target bounds, scoped character collision-region anchors, deliberately interactable-object bounds, onscreen hostile-character props, and the existing query-ray hit coordinate/body part before prop rendering; observe after player sight/HUD rendering; expose read-only multiplayer limit getters | Finite projected bounds and hit-region centers, final filtered `lookingatprop`, exact query hit and model hit part, native sight state, player viewport, rendered interactable/character membership, and time/player/team score limits | PC prop rendering converts float model matrices in place before sight/HUD state is final, so post-render beacon ticks cannot safely project terminals or sniper anchors and one targeting hook cannot obtain both targeting states; status queries must not duplicate private limit globals | Adjacent bounded beacon and targeting capture calls plus the later targeting observation support viewport-true scanners, firing-range and generic hostile-character profiles; scoped guidance retains five fixed collision-region anchors without adding another hook or query. Query results survive final semantic filtering. Three pure getters supply the F1 status adapter with authoritative limits |
