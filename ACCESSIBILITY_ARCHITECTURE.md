...
| `port/src/audio.c` | Mix an enabled procedural accessibility tone into each completed stereo buffer before SDL queueing | Tone frequency and enabled state only | A clean responsive carrier cannot be made from game samples with finite duration or baked-in modulation | Integrated into the targeting alignment lane; fixed buffer, phase continuity, frequency interpolation, and gain ramp require targeting acceptance testing |
