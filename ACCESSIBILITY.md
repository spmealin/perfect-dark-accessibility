...
- Door candidates must be rendered in the active camera view and have a clear background line of sight on every scan and again before every chirp. A sibling group representing paired door leaves retains one canonical identity and selects at most one rendered leaf as its sound source. Interactable-object candidates require background line of sight without the door-specific on-screen rule; proximity and room adjacency alone are never sufficient. The scanner first tests the object's origin, then, for recessed objects whose origin is inside supporting level geometry, tests five bounded points on the nearest face of the object's model box. If all ordinary rays fail, an object that was actually rendered on-screen and whose native interaction does not request its own LOS check receives one final set of rays with those surface endpoints pulled eight world units toward the camera. This narrowly tolerates wall-mounted monitors whose complete model box is embedded behind its supporting wall; it does not admit an off-screen object or replace the collision ray with viewport evidence. Collectible items use the pickup path's door-and-background sight blockers. The selected target does not block itself, while intervening world geometry suppresses targets beyond it.
