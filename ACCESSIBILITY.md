...
Pointing directly at an admitted hostile starts the responsive alignment tone, including while unarmed; combat alignment currently uses the low 660 Hz end of the pitch range because semantic head/high-value-zone quality is not yet supplied. Characters retain the game's finalized character-target selection. Automated turrets and security cameras use the same-frame non-random weapon query ray already used to determine where a shot would land. This supplies feedback only on direct crosshair intersection and does not pull, snap, or otherwise alter aim. Characters in dying, dead, knockout-fall, or knocked-out actions and cameras that become disabled, deactivated, or destroyed are removed from the audible combat slots. Firing-range targets retain their scoring-center pitch mapping, procedural round robin, and back-facing shootability rule. Switching between firing-range and combat profiles clears prior identities and owned sound state.
