...
Pointing directly at an admitted hostile starts the responsive alignment tone, including while unarmed; combat alignment currently uses the low 660 Hz end of the pitch range because semantic head/high-value-zone quality is not yet supplied. The lock is solid for hostile characters, automated turrets, security cameras, threat-detector objects, firing-range targets, and validated device targets. Protected/non-hostile characters, breakable route-obstruction glass, and destructible containers that own a collectable item retain the same alignment pitch but interrupt it with an exact 10 ms silent gap in every 100 ms cycle. The 90 ms sound/10 ms gap pattern quickly distinguishes a valid target that is not an ordinary hostile from a solid enemy lock without changing semantic eligibility or pitch. Loot-container detection follows the engine's parent/child ownership rather than a stage or model allowlist: the outer object must be active, visible to the exact attack query, healthy, mortal, and contain an `OBJFLAG_INSIDEANOTHEROBJ` collectable child. Empty, destroyed, invincible, invisible, and uncollectable containers are excluded, and the equipped attack must be capable of damaging the outer object.
...
EnemyScopedFullVolumeDistance=5000.000000
EnemyScopedFadeDistance=5800.000000
EnemyScopedMaximumDistance=6000.000000
