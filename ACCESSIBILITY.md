...
Pointing directly at an admitted hostile starts the responsive alignment tone, including while unarmed; combat alignment currently uses the low 660 Hz end of the pitch range because semantic head/high-value-zone quality is not yet supplied. The lock is solid for hostile characters, automated turrets, security cameras, threat-detector objects, firing-range targets, and validated device targets. Protected/non-hostile characters and breakable route-obstruction glass retain the same alignment pitch but interrupt it with an exact 10 ms silent gap in every 100 ms cycle. The 90 ms sound/10 ms gap pattern quickly distinguishes a valid target the player may not want to shoot from a solid hostile lock without changing semantic eligibility or pitch.

Characters retain the game's finalized character-target selection. A two-frame release grace keeps the tone stable across a transient one- or two-frame loss only while that same living character remains visible, hostile, shootable, and unobstructed; a different target, invalid state, scope transition, or a third lost frame stops or changes it immediately. Automated turrets, security cameras, threat-detector objects, and breakable route obstructions use the same-frame non-random weapon query ray already used to determine where a shot would land and receive no release grace. This supplies feedback only on direct crosshair intersection and does not pull, snap, or otherwise alter aim. Characters in dying, dead, knockout-fall, or knocked-out actions and cameras that become disabled, deactivated, or destroyed are removed from the audible combat slots; detector objects disappear when the native threat list removes them or the player leaves the detector function. Firing-range targets retain their scoring-center pitch mapping, procedural round robin, and back-facing shootability rule. Switching between firing-range and combat profiles clears prior identities and owned sound state.
...
- Door candidates must be rendered in the active camera view and have semantic visual line of sight on every scan and again before every chirp. Interactables, pickups, and people use the same visibility rule without the door-specific on-screen requirement during Joanna's ordinary view. Background, closed doors, opaque objects, and opaque path blockers suppress the target; props marked `OBJFLAG_AISEETHROUGH`, including semantically transparent glass, do not. A sibling group representing paired door leaves retains one canonical identity and selects at most one rendered leaf as its sound source. Door visibility excludes every leaf in that same sibling group from the synchronous ray and tests the selected leaf's origin plus five bounded points on its nearest model-box face; this prevents either an embedded anchor or the other half of a doorway from concealing the doorway itself. The scanner uses the same origin and five surface probes for interactable objects. If all ordinary interactable rays fail, an object that was actually rendered on-screen and whose native interaction does not request its own LOS check receives one final set of rays with those surface endpoints pulled eight world units toward the camera. A monitor carrying the native `OBJFLAG_MONITOR_RENDERPOSTBG` placement semantic uses a bounded 24-unit retry because those pads deliberately place the visible screen plane farther inside the mounting geometry. The larger allowance does not apply to ordinary interactables, and every retry must still pass the semantic collision ray; on-screen state alone cannot admit a target. Intervening opaque geometry remains authoritative.
