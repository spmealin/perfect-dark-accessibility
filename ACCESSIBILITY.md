...
While the Sniper Rifle is equipped and its live FOV is narrower than the configured normal FOV, the closest eligible hostile to the crosshairs receives precision guidance through its existing combat voice. It repeats a 100 ms chirp every 160 ms. Rather than steering toward the midpoint of the character's coarse overall projected rectangle, the adapter projects centers from the active model's existing collision boxes and retains fixed head, torso, arm, lower-body, and other-region anchors. Gun and hat boxes are excluded. It prefers the anchor nearest the crosshairs with a small bias toward the body region that supplied the successful semantic visibility sample; if a character model exposes no usable collision-box anchor, the engine's native auto-aim position is the fallback. Horizontal anchor error, normalized against the current narrowed viewport, drives exaggerated stereo pan; the first half remains at the normal enemy carrier and the second half rises when that anchor is above the crosshairs or falls when it is below. A small center dead zone resolves to the unmodified carrier. Target selection retains the current hostile until another is meaningfully closer to the crosshairs, avoiding rapid switching between adjacent enemies. Anchor capture uses five fixed slots per projected character and allocates nothing. This guidance still never creates a lock: only the native/raw semantic aim result can start the separate solid 660 Hz alignment tone, and model polygon tests can still reject a ray through an individual collision-box center.
