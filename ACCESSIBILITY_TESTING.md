...
At an unrailed edge, a missing floor or downward change greater than the default 80 units must produce a distinct 260 ms steep falling contour. Approach slowly and confirm that its spatial source remains at the floor-to-drop transition rather than moving to the player or a wall across the gap. Test straight and diagonal approaches on a suspended walkway: side rays should expose nearby edges while a supported forward ray remains clear. A wall beyond the edge must not mask the nearer drop; a railing or closed barrier before the edge must remain a barrier cue and must not expose an unreachable drop. Isolated room-resolution misses must not warn unless refinement obtains at least two unsafe samples. Compare `state=drop`, `drop`, `drop_refinements`, `drop_threshold`, `terrain_distance`, and probe reasons `drop_candidate`, `edge_safe`, and `edge_unsafe` with the perceived boundary.
