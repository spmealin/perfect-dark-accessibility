...
InterruptedTargetingVolume=0.12
...
VirtualCaneDropHeightThreshold=80
...
Source review confirms that the implementation is inert unless both top-level accessibility and `Accessibility.InteractableBeacons=1` are effective. In unobscured gameplay, F5 independently toggles interactable-object beacons, F6 toggles door beacons, and F8 toggles pickup beacons; any combination can be active. F5/F6 retain their existing repeat/cancel meanings in menus, while F8 is ignored there. The scan supports any one-local-player mission or Combat Simulator match and is limited to active props, the current/directly-adjacent room boundary, a 1,200-unit radius for ordinary categories, and a 3,000-unit radius for rendered doors. Interactable objects share the broad predicate used before `objTestForInteract` applies range/facing, including CI tags, alarms, thrown laptops, Hacker Central terminals, explicit interactables, lift controls, and movement-state-eligible vehicles/grabbable props. Pickups use the engine's object-type and collectable/uncollectable semantics; doors are canonicalized through sibling links. Scanning does not call the interaction or collection actions, so it cannot select, collect, mount, grab, or activate a target.

In Area 51: Rescue, approach the intact wall marked with the silver X with `Accessibility.AuthoredLandmarks=1`. The tagged `OBJ_WALL` prop must produce the continuous player-marker base drone without any 800 Hz identity chirps. It is independent of F5 and must not consume an interactable result or chirp-scheduler slot. Confirm it begins only within `Accessibility.MarkerRange` with direct visual line of sight, spatializes at the tagged wall, continues when F5 is toggled off, follows the CamSpy observer if that perspective can reach the area, and stops when occluded, out of range, destroyed, paused, or in a menu. Place F9–F12 markers nearby and count their one-to-four chirps; the authored landmark must remain the chirpless voice and must not disturb their identity schedule. Confirm the wall still creates no weapon lock tone and that unrelated non-activatable scenery remains silent.
...
In Skedar Ruins, approach the three pillars selected by the level's native shuffle for "Identify temple targets." Setup tags `0x01`, `0x02`, and `0x03` must independently produce the chirpless mission-landmark drone while visible, in range, and unmarked. Attach a Target Amplifier correctly to one pillar and confirm that pillar's landmark stops immediately while the remaining two continue. Repeat for all pillars; each must use its matching `STAGEFLAG_PILLAR*_MARKED` state rather than waiting for the whole objective to complete. With the R-Tracker active, a completed pillar must also leave the accessibility tracker candidate list even though the single-player stage script retains `OBJFLAG3_RTRACKED_YELLOW`; an unmarked pillar outside landmark line of sight must remain available through the R-Tracker. Session `1789230677`, capture 1 confirms stage 42's shuffled tag `0x03` pillar, model 396, was healthy, on-screen, and carried the yellow tracking flag before this landmark coverage was added.

During the Skedar Ruins final boss, verify that the four lower spike props (setup tags `0x13`–`0x16`) produce independently positioned mission-landmark drones only while the game has made them vulnerable. No spike may sound while its native `OBJFLAG_INVINCIBLE` state is set. Destroy one lower spike and confirm its source stops without restarting or moving the remaining three voices. After all four lower spikes are destroyed, verify that the upper spike (`0x17`) begins sounding only when the game makes it vulnerable. Surviving sources must stop when a vulnerability window closes and resume, with staggered starts, when the next window opens. Range, line of sight, pause/menu suppression, and CamSpy observer behavior must match other authored landmarks. Confirm that the earlier completed target-pillar registry entries consume no voices during this encounter and that no capacity warning appears in the accessibility log.
...
For F7 viewport acceptance, keep a living friendly or neutral character within range and direct line of sight, then rotate until the renderer no longer marks that character on the current screen. Its dedicated drone must stop during pre-playback validation and remain absent from the next scan. Rotate back until the character is rendered and confirm the drone returns without toggling F7. Repeat from the CamSpy perspective and correlate exclusions with `character_not_rendered_on_screen` or `character_not_rendered_in_remote_view`.
...
Door audio now bypasses the object/pickup round-robin. Test with F5 and F6 enabled while at least three computers and one to three doors are visible: every retained door must repeat on a shared 375 ms cadence and must not slow down as interactable results enter the schedule. With multiple doors, starts must occupy the fixed 0, 125, and 250 ms windows; each 100 ms chirp leaves a 25 ms gap. Bring a second or third door into view during another door's active chirp and confirm the new cue waits for its next complete window rather than starting partway through or overlapping. Stand centered before single-leaf and paired doors, then open and close them repeatedly: the cue must remain at the stable doorway center rather than following a sliding/swinging leaf or jumping when another sibling becomes the retained visible leaf. Visibility and line of sight must still follow the actually rendered door geometry. Verify the louder 0.24 backend level, extended 3,000-unit discovery range, and proportional 416.7/2,666.7/3,166.7-unit attenuation thresholds remain useful without admitting off-screen, occluded, or duplicate sibling leaves. Correlate `door_chirp_assign`, `door_chirp_release`, `door_chirp_stop`, `door_chirp_slots`, and performance `door_enabled_slots` fields; all counts must remain at or below three and return to zero on suppression, toggle-off, stage teardown, and shutdown.
...
an enemy through intact glass: the enemy presence cue may sound, and exact aim
on the enemy must produce the penetrable-glass lock modulation described below
...
Confirm the session-start record reports `non_hostile_beacons=1`. In Carrington Institute gameplay, leave F5/F6/F8 off, press F7, and approach known neutral staff. Every retained neutral or protected person should emit a continuous positioned chord with a steady 440 Hz fundamental and quieter continuous 550 Hz major third. Approach a character classified as friendly and confirm the 440 Hz component remains steady while only the 550 Hz component alternates one second on and one second off without clicks. Confirm `friendly_drone_assign` reports the expected relationship and `third_pulsed` value. Trigger a scripted friendly/neutral relationship change where practical and verify `friendly_drone_reclassify` restarts the upper-component pattern without losing the positioned fundamental. Confirm both variants remain readily distinguishable from one-chirp doors, three-chirp pickups, hostile pulses, and player markers' moving 300–600 Hz base plus 800 Hz identity chirps. Toggle F7 off and confirm all character drones fade out promptly without affecting hostile targeting or other beacon categories. Then enable doors, pickups, people, and at least one player marker together and confirm their identities remain unambiguous.
...
Regression-test session `1786059687`, capture 1, or equivalent close-range geometry. Door prop 39 was approximately 197 units away and carried the engine's coarse `onscreen=1` flag despite a recorded bearing near 114 degrees and no visible geometry in the viewport. Confirm it now logs `door_outside_viewport` and remains silent. Rotate until any edge of its projected model rectangle enters the viewport and confirm one cue starts; rotate until the entire rectangle leaves and confirm pre-chirp validation releases it without waiting for the twice-per-second rescan. Repeat at the top and bottom viewport edges and with each leaf of a paired doorway. Diagnostic records must show `projection_found=1`, finite bounds, and `intersects_viewport` matching the audible result.

For native reactive-object locks, reproduce Air Base diagnostic session `1786399911`, capture 2. Aim at the intact robot interceptor (prop 146, `OBJTYPE_HOVERPROP`, model 296), whose `OBJFLAG3_REACTTOSIGHT` makes the native reticle blue/red. The exact crosshair hit must now produce the interrupted 396 Hz lock (`category=10`, `alignment_source=native_reactive_reticle`, `pattern=90ms_on_10ms_off`) without adding an object-presence voice or forgiving target margin. Destroy it and confirm the native sight rejection and accessibility lock both stop. Repeat with reactive breakable glass and representative destructible scenery, then aim at ordinary scenery whose reticle does not change and confirm it remains silent. An object already classified as a path blocker, loot container, turret, camera, threat, or device target must retain its more specific category rather than being duplicated.
...
children. Each exact attack-query hit must produce the interrupted 396 Hz lock
...
Enable distinct combinations of F5, F6, F7, and F8, then complete, abort, restart, and advance between consecutive missions and Combat Simulator matches. Every stage teardown must stop the active chirp and all friendly drones, then discard all prop identities, result indices, schedules, drone slots, observer pointers, and timing state while retaining the exact four category selections. Eligible gameplay in the next stage must perform a fresh scan and resume those categories without a toggle earcon. Verify that a category deliberately left off remains off, including F7 when moving into or out of a mission where non-hostile cues are unwanted. Stage teardown and shutdown must clear transient output while retaining the registered selections, and no pointer or prop number from the prior stage may appear in the new schedule or drone assignments.

After choosing a distinct F5–F8 combination, exit the game normally and inspect `pd.ini` for `Accessibility.InteractableScannerActive`, `Accessibility.DoorScannerActive`, `Accessibility.PickupScannerActive`, and `Accessibility.NonHostileScannerActive`. Restart, enter eligible gameplay without pressing a scanner key, and confirm exactly that combination resumes with fresh targets and no startup toggle earcon. The `lifecycle/scanner_start_state` record must match all four `saved` values and report the same `effective` values while both scanner backends are enabled. Repeat with all off and all on, then disable each backend master and confirm its saved selections remain intact while its effective fields and output are off. Confirm Alt+F4 and the normal exit menu both reach orderly configuration saving; forced process termination is not required to preserve unsaved changes.
...
With `Accessibility.HudMessages=1`, start Carrington Institute holo-training session 1 and confirm its instructional HUD messages speak automatically in displayed order without interaction. Trigger another ordinary HUD message elsewhere to verify the common hook is not training-specific. Pick up two identical items quickly enough that the native visual queue suppresses the second duplicate and confirm both semantic events produce ordered speech requests. Confirm an in-game subtitle/dialogue and a cutscene subtitle are not spoken by this HUD feature, while `hud/message_observed` and `hud/speech_suppressed reason=subtitle` retain their metadata. Check multiline/control whitespace is spoken as one normalized sentence, and confirm menu focus speech can interrupt queued HUD speech without preventing a later HUD message. Repeat with `HudMessages=0` and confirm observed messages are logged with `reason=feature_disabled` but not spoken.
...
Equip an ordinary pistol or rifle, hold manual aim, and place one visible hostile around each edge and then near the crosshairs. Repeat with a scoped weapon at more than one zoom level. In every case the selected hostile must chirp for about 100 ms every 160 ms; left/right screen error must reach useful stereo separation relative to the live viewport, and the second half must rise for its selected point above the crosshairs or fall for one below. Release manual aim and confirm the voice immediately returns to ordinary distance/elevation cadence. Rebind aim or use another supported control scheme and confirm activation follows the semantic aim action rather than a hard-coded left-trigger input. Outside the expanded target rectangle, logs should show coarse `model_hitbox` guidance without detailed queries. Enter the 12-pixel envelope and verify one target reports `precision_fine_attempted=1`; when a detailed surface validates, `precision_anchor_source=model_geometry` must replace the coarse anchor and guide the crosshairs toward a point from which the independent native lock can be acquired. Centering vertically must restore one uniform normal enemy carrier.

Repeat manual-aim acquisition against an active hostile automated turret and a targetable security camera. Each must participate in the same closest-crosshair selection and 100/160 ms exaggerated pan/pitch guidance. Outside manual aim, the turret must return to ordinary enemy cadence and the camera to its distinctive 1,600-to-1,000 Hz scan. Confirm `model_hitbox` or `model_geometry` supplies the guidance point when the object exposes collision nodes; `model_bounds_fallback` is acceptable only when no usable node exists. Move from the guidance point to an exact native/raw object hit and verify the centered lock tone starts independently. The turret's existing three-pixel tolerance may still produce its lock, but precision guidance itself must not widen camera collision, redirect shots, or claim a hit.

Move across standing, crouched, animated, and partially covered guards, including the distant sniper configuration from diagnostic session `1785715598` capture 1 and prop 27 from session `1785717968` capture 1. The latter previously brought the crosshairs within about 1.3 horizontal and 5.8 vertical pixels of a left-thigh box center at frame 10202, and within about 4.5 by 7.4 pixels of a head-box center at frame 12662, while the raw query remained null. The refined cue must reject such empty or occluded samples, select `model_geometry` when another sampled surface validates, and allow the solid lock to be reached without making the guidance tone itself a hit claim. With two adjacent enemies, move slowly across their midpoint and confirm only one is refined per frame and the retained identity does not chatter; a meaningfully closer target must take over. Leave zoom, switch weapons, acquire an exact lock, occlude or eliminate the target, and verify detailed queries stop or return that combat slot to ordinary distance cadence.

Correlate `precision_refinement`, `precision_guidance_select`, `precision_guidance_stop`, detailed `combat_candidate`, and `cue=manual_aim_precision` records. Query count must never exceed 15 per refinement; repeated `budget_exhausted=1` without a later valid point requires more sampling analysis rather than silently increasing the budget. In an advanced-diagnostics build, compare matched stationary aimed and unaimed scenes with the crosshairs outside and inside the fine envelope, including both ordinary and zoomed weapons. Use `performance/targeting_window` to calculate query microseconds per rendered frame and per detailed query while comparing render FPS and maximum frame gaps. Flag a sustained FPS loss above five percent, more than one millisecond of average refinement work per rendered frame, or repeated refinement maxima coincident with slow-frame gates. Exercise at least one unusual character model; `native_autoaim_fallback` is acceptable only when no usable model collision anchor was captured.

Blind-user acceptance on 2026-08-02 reported that the two-pass guidance worked in session `1785719577`. Across 4,727 refinement frames, 4,726 found valid geometry, 11,363 detailed rays averaged 2.40 per refinement, and only one animated/partially visible frame exhausted the 15-query budget; a nearby refinement recovered a valid torso point with 14 queries. Refinement-active one-second windows averaged 60.006 FPS and never fell below 59.841 FPS. They accumulated 86,875 microseconds of refinement work, about 18.4 microseconds per active frame, with a 744-microsecond lifetime maximum that did not coincide with a slow frame. No graphics slow-frame episode, crash, targeting-capacity overflow, or memory change correlated with refinement activity was recorded. This is positive functional and performance evidence for the tested encounter, not full acceptance across character models, animation states, and levels.
...
Multiple enemies should retain independent spatial voices whose starts are staggered rather than exactly simultaneous. While unarmed, point the hidden camera-center melee query across enemies, protected characters, breakable glass, and collectable-bearing crates: positioned enemy presence and distance cues must continue, but no centered lock or threat-aim repetition may play. Logs must report `indicator_visible=0`, `alignment_permitted=0`, and `shootability_reason=target_indicator_hidden`; an existing lock must stop immediately with the same reason if the indicator becomes hidden. Equip an ordinary weapon whose native target indicator is visible and move the exact query ray across distinct model regions: head geometry must produce an 825 Hz centered lock, either hand/forearm/bicep must produce 528 Hz, and torso, pelvis, legs, feet, or other character geometry must retain 660 Hz. With Sight On Screen and Always Show Target enabled, repeat without holding fine aim and confirm the visible native target marker still permits alignment; in particular, use the Laptop Gun in the CI firing range and confirm centered shootable targets produce its quality tone. Disable Sight On Screen and confirm alignment is suppressed. The change must follow the model hit part rather than estimated screen position and must not change where a shot lands. Repeat on a protected/non-hostile character where safe: the interrupted lock must use about 317 Hz for arms, 396 Hz for torso/other regions, and 495 Hz for the head, while glass and collectable-bearing crates use the interrupted 396 Hz object lock. Thus even the highest interrupted body cue remains below the ordinary hostile arm cue at 528 Hz. Firing-range scoring quality and non-character device/object locks must remain unchanged when their active profile exposes its target indicator. Correlate `targeting/scope_gate` fields `raw_aim_hitpart` and `raw_aim_region` with `targeting/alignment_start` or `alignment_update` fields `aim_region`, `frequency_hz`, `pitch_multiplier`, `volume`, and `volume_source`. Move narrowly off and immediately back onto the same still-visible character: a one- or two-frame query loss may retain the tone, must log `aim_loss_grace_start` and `aim_loss_grace_cancel`, and must not generate a stop/start pair. Remain off aim and confirm it stops no later than the third observation. Moving directly to another valid target must switch immediately without inheriting the grace period. Verify moving characters reacquire automatically. Knock out all three training opponents and confirm each voice stops as soon as the knockout fall begins and remains absent while the body lies on the floor. Test partial cover, full occlusion, turning away, pause/menu state, training completion and abort, and several sessions without restarting.

Verify the two centered-lock volume controls independently. With `Accessibility.TargetingVolume=0.15`, test `Accessibility.InterruptedTargetingVolume` at `0`, `0.12`, and `0.4`: protected characters and interrupted breakable/reactive/loot-container locks must be silent, default-level, and bounded-maximum respectively, while ordinary hostile, turret, camera, threat, firing-range, and validated-device solid locks remain unchanged. Then hold `Accessibility.InterruptedTargetingVolume=0.12` and test `Accessibility.TargetingVolume` at `0`, `0.15`, and `0.4`; only the solid family must change. Neither value may scale, inherit, or fall back to the other. A hostile acquired through penetrable glass retains the solid family volume and body pitch with the obstruction tremolo; an interrupted protected target through penetrable glass uses the interrupted family volume and lowered carrier with both patterns. Move directly between solid and interrupted targets and listen for clicks, stale gain, or a pattern that changes without the matching gain. The session-start record must report both effective values, and alignment records must identify the selected `volume_source`.

At a native windowed door, aim through the visible pane at an admitted hostile. Exact aim on the target must produce the ordinary body-region carrier with a clearly audible 12 Hz amplitude tremolo; head, torso, and arm hits must retain their respective 825, 660, and 528 Hz pitches. Move the crosshair from the pane onto the opaque frame without leaving the target's projected area and confirm the lock stops rather than passing through the whole door. Reacquire through the pane, open or break the glass where possible, and confirm the tremolo disappears while the clear-path lock retains the same body pitch. Repeat with standalone breakable glass. Invincible or bulletproof glass, an opaque door panel, another solid prop beyond the glass, and background geometry must not expose a target behind them. Device targets and firing-range scoring must retain direct-hit semantics. Correlate `scope_gate` values `raw_aim_obstruction=1` and `alignment_source=raw_query_through_glass` with `alignment_start`/`alignment_update` values `obstruction=1`, the body `aim_region`, unchanged carrier `frequency_hz`, and `pattern=penetrable_tremolo`. Confirm the calculation adds no second world trace and does not change native firing, damage, `lookingatprop`, auto-aim, or reticle behavior.
...
In Chicago, collect and equip the mission-renamed Data Uplink (`Reprogrammer`) and face the objective hovercab. Confirm that only native taxi tag `0x0a` produces the fixed 660 Hz alignment tone when the raw aim ray hits it; the other decorative taxis and vehicles must remain silent. Activate it from valid range and verify the mission script remains authoritative for the connection, completion, and already-programmed states. Logs must show `stage=STAGE_CHICAGO`, `profile=3`/`source=3`, `weapon=54`, `target_tag=10`, raw aim identity, and clean acquisition/loss when aiming away or changing weapons.

In G5 Building, equip the Door Decoder and face the safe keypad. Confirm that only native keypad tag `0x11` produces the fixed 660 Hz alignment tone when the raw aim ray hits it; nearby laser switches, alarms, monitors, and the safe door must remain silent. Activate the keypad from valid range and verify the native script remains authoritative for attaching and removing the decoder, cracking progress, alarms, and unlocking the safe. Logs must show `stage=STAGE_G5BUILDING`, `profile=3`/`source=3`, `weapon=57`, `target_tag=17`, raw aim identity, and clean acquisition/loss when aiming away or changing weapons.

Complete the campaign matrix in `documentation/ACCESSIBILITY_SPECIAL_DEVICE_TARGET_AUDIT.md`: Defection's Uplink PC; Infiltration's Comms Rider antenna and Explosives terminal; all three Rescue Uplink terminals; Escape's Auto-Surgeon hoverbed; Chicago's two Remote Mine fire-door surfaces and Tracer Bug limousine; Defense's Uplink shuttle; and all three Skedar Ruins Target Amplifier pillars. For every row, test the listed target, a visually similar wrong prop, weapon equipped/unequipped, exact aim/off aim, valid/invalid native use, completion or item removal, pause/menu, death/restart, and stage exit. Exercise every difficulty and co-op variant that exposes the item. The tone must follow target identity without claiming range, trajectory, attachment, or mission success. Chicago's two mine rows must log `expected_prop_type=2` (`PROPTYPE_DOOR`); all other current static rows must log `expected_prop_type=1` (`PROPTYPE_OBJ`). Any `wrong_prop_type`, `target_unavailable`, or unexpected persistent eligible row after completion requires investigation before acceptance.

Also sample the audit's deliberate exclusions: Deep Sea's Backup Disk near Dr. Caroll, Mr. Blonde's Revenge Skedar Bomb placement room, Crash Site's President Scanner, and representative carried/pickup-only mission items. They must not enter the static device profile merely because they are equipped. Validate their native or dedicated scanner interaction instead. This negative pass guards against turning proximity areas, characters, and inventory delivery into false point targets.
...
of continuation must still yield to that wall. A connected crouch opening must
also prove two player radii beyond its first lower-clearance node; confirmed
openings may replace a legacy wall and are positioned one radius inside the
opening, while short or uncertain paths retain the established result. Ladder,
...
`terrain_validation`, `crouch_validation`, `legacy_drop_distance`, `cue_distance`, `cue_delta`,
...
Confirm `cane/sweep` reports the blocking prop for diagnosis and records the
ordinary wall duration and matching start/end frequencies. Use the separate
`targeting/path_blocker_aim` event when diagnosing whether that prop is
breakable. Aim an ordinary firearm at
...
For terrain contours, approach upward and downward stairs, a shallow and steep ramp, a landing, uneven but effectively flat floor, and a sheer edge from several headings. Reproduce the DataDyne staircase regression from session `1784916426`: near position `150.96,-1334.81,-599.92`, the center ray must resolve the lower stair room and report the downward contour before the former late boundary near `146.26,-1357.92,-683.66`. The first height change of at least 12 units within 450 units must produce a clearly rising 140 ms contour for rising terrain and a clearly falling 140 ms contour for descending terrain at the correct horizontal angle. Ordinary barriers must retain their steady 35 ms chirp.

For the connected terrain-contour prototype, reproduce DataDyne Investigation session `1788989589`, capture 1. Sweep 410, center ray `s4`, observed flat floor from 0 through 210 units, descending stairs from 240 through 420, a lower landing at 450, and a movement-blocking glass surface around 648. One uninterrupted tone must begin at the starting-floor pitch, hold over the near flat, glide proportionally lower through the descent, hold again over the lower landing, then—after 5 ms—end with the harder, quieter wall impact. The final barrier must remain present even though it lies beyond the 450-unit floor-query reach. Test an upward equivalent, a ramp that reaches open level ground, and a long stepped profile. Confirm the contour's volume fades with distance, equal absolute elevations hold equal pitch, and there are no audible gaps, attacks, or phase resets at internal knots. No contour may exceed ten 32 ms time units; long paths must be spread across the fixed capacity while retaining a genuine final landing. `path_contour_count` and `path_contour_floor_count` must remain zero for barriers, drops, crouch passages, ladders, uncertain paths, and fallback terrain. For a connected terrain contour, compare every `path_contour` entry's `type=floor|wall`, distance, elevation, frequency, and volume with the perceived contour; `terminal_wall` must agree with whether the total count exceeds the floor count. This contour communicates sampled geometry, not assurance that the route is usable beyond its endpoint.

For surface-plane arbitration, first approach a ramp from flat ground: its beginning must remain a spatial terrain contour. Stop midway on a steady ramp and face along it. The nine rays must no longer repeat that same grade when their floor samples follow the current plane; openings should become silent and walls beyond the slope should sound at their collision positions. No centered ascent/descent tone or `cane/grade_context` event should occur. Walk across a crest or slope boundary and turn during both a Slow and Fast sweep: later ray classification must use the logged sweep-start plane in absolute world coordinates. At the crest and bottom, plane deviations must restore spatial transition cues. Repeat on discrete stairs, native step-flagged geometry, a low-headroom ramp, a non-traversable rise, a descending ramp, an uneven hillside, and a large drop; stairs, unsafe transitions, and drops must retain their established spatial semantics. Confirm grabbed-object, CamSpy, and hoverbike profiles do not apply walking surface-plane suppression. Inspect `surface_point`, `surface_normal`, `surface_ground_delta`, `forward_grade_ratio`, `surface_grade_ratio`, `grade_max_residual`, `grade_continuous`, `grade_safe`, and `terrain_suppressed` in `cane/sweep`, and verify the single current-floor query does not produce a measurable sustained frame-time regression.

At an unrailed edge, a missing floor or downward change greater than the default 80 units must produce a distinct 260 ms steep falling contour. Approach slowly and confirm that its spatial source remains at the floor-to-drop transition rather than moving to the player or a wall across the gap. Test straight and diagonal approaches on a suspended walkway: side rays should expose nearby edges while a supported forward ray remains clear. A wall beyond the edge by more than one live player collision radius must not mask the nearer drop; a railing or closed barrier before the edge remains authoritative. Reproduce session `1788528042`, capture 1 near `-754.14,-928.00,5338.76`: the rightmost ray refined a 294-unit floor drop about 28 units away while movement collision found a wall only about two units farther. That inaccessible lower floor must now produce the wall cue with `drop_barrier_suppressed:1`, a positive `drop_barrier_gap` no greater than `bbox` radius, and `terrain_suppressed:1`. Repeat with a truly open ledge and walls just below, at, and above the one-radius boundary to confirm only collision-inaccessible gaps are suppressed. Isolated room-resolution misses must not warn unless refinement obtains at least two unsafe samples. Compare `state=drop`, `drop`, `drop_refinements`, `drop_threshold`, `drop_barrier_suppressed`, `drop_barrier_gap`, `terrain_distance`, and probe reasons `drop_candidate`, `edge_safe`, and `edge_unsafe` with the perceived boundary.

At a known crouch-only opening, approach while standing and confirm the blocking ray produces two 55 ms chirps separated by 25 ms at the opening, with the second chirp clearly lower than the distance-pitched first. Reproduce session `1786057931`, capture 1 near `149.562,-1899.000,-513.000`: center rays found rising terrain 339–348 units away only one unit before the standing barrier, so they must now run the squat test and prefer a confirmed co-located passage rather than allowing the stairs to mask it. Verify full squat can actually traverse the sampled opening, an ordinary wall retains its wall cue, and an obstacle immediately behind a low opening prevents a false passable result. Repeat while ducking, fully squatted, beside rather than facing the opening, and in CamSpy. Large-drop priority must remain unchanged. The cue must not mutate stance or collision state, and CamSpy or an already full-squat collision envelope must not claim crouch semantics. Correlate `state=crouch`, `crouch`, `crouch_terrain_merge`, `tone_pattern`, `crouch_result`, `crouch_pass`, and `crouch_ymax` with the terrain, standing, and squat-height outcomes. Confirm ordinary distance still controls the first carrier's pitch and attenuation. Record false positives from floor seams, decorative geometry, stacked rooms, lifts, and nearby lower floors.

At the Air Base ladder represented in diagnostic session `1786399911`, capture 1, face and approach the climbable surface from several angles. Every cane ray that identifies native ladder geometry must produce a continuous 220 ms upward sweep at the impact point. Its starting carrier must match the lowered start of the ordinary stair-up contour, but its endpoint must reach twice the distance carrier and therefore sound substantially higher than stairs. Adjacent ordinary wall geometry must retain the single wall chirp. Confirm the cue preserves distance-dependent pitch, attenuation, and spatial pan, and that `cane/sweep` reports `state=ladder`, `ladder:1`, start/end frequencies with the documented ratios, and the continuous contour pattern. Repeat at the DataDyne Research ladder represented by session `1788980579`, capture 1: the direct rays reported ordinary `geoflags:0x1c`, so the player-sized barrier-contact probe must report `ladder_probe_hit:1`, one or two bounded `ladder_probe_queries`, and the contact distance and native normal while producing the ladder cue. Session `1788981635` confirms that probe repeatedly acquired the ladder with one query; blind-user acceptance of the revised sound remains pending. Adjacent wall rays must report a probe miss and remain barriers. Repeat with player-only ladder geometry where available and verify unflagged decorative rungs do not claim climbability.

Reproduce Air Base session `1786484131`, capture 2 at approximately `4320.56,69.63,3128.38`. A short traversable rise begins 14–45 units ahead and settles near ground zero while fence geometry lies roughly 130–160 units away on the side rays; the center ray continues through the fence opening. Confirm side rays now report the farther fence wall while the center becomes silent, with `traversable:1`, `plateau:1`, and `terrain_suppressed:1` where applicable. Repeat while standing, ducking, fully squatted, and during each stance transition: `stance`, the logged live `bbox`, and clearance results must change together, and terrain may be suppressed only when that exact envelope fits. Add a low ceiling that blocks standing but clears duck or squat and confirm classification changes without mutating stance. Finally test short ledges without a farther wall, sustained stairs and ramps, non-traversable high steps, downward slopes, and drops. Only the nearby clear plateau should become silent; stairs/ramps and all hazards must retain their established contours. Verify clearance queries remain at or below four per ray and inspect advanced cane timing for regression.
...
For remote cane collision coverage, hold the CamSpy still while all nine angles complete across doorways and overlapping room boundaries. Every scheduled sample must resolve to collision, no collision, or terrain rather than `result=-1`. Repeat as Joanna at an overlapping room or portal boundary. A destination-room disagreement may record one `barrier_retries` or connected-path `retries`, but the portal-walk retry must resolve it; recurring nonzero `errors` or a remaining `result=-1` is a defect. Confirm `remote_floor_room`, `remote_floor_simplified`, `remote_rooms_expanded`, `observer_rooms`, `initial_dst_rooms`, and `final_dst_rooms` describe the native CamSpy room preparation, and that `ignored_remote_prop` identifies the active CamSpy while its collision perimeter is temporarily excluded. Return to Joanna and verify these remote-only fields no longer appear, her collision perimeter remains enabled, and ordinary cane results are unchanged.

For corridor-aware terrain, approach narrow stairs and ordinary ramps approximately two to five player widths across, including a descent that the path query recovers while disproving a false drop. Confirm the ordinary per-ray elevation contour is absent and, after the directional sweep, the boundary sequence traces all available left stations near-to-far from 330 Hz, then all right stations near-to-far from 440 Hz. Consecutive uncertain rays before or after a bounded or guided ray for the same ascent or descent must report `terrain_audio_held:1` and must not layer the legacy contour over the boundary sequence; an isolated all-uncertain terrain group with no positively located hard boundary must retain one fallback contour. Both sides must rise or fall together according to the route elevation while retaining their pitch separation. Near clicks must be louder than otherwise equivalent far clicks, and every click must retain the pan of its individual boundary position. Test a staircase that ends on a landing with a wall continuing left and an opening on the right: both boundary traces must descend the stairs, the left trace must continue along every positively detected landing-wall station, and the right trace must terminate where its boundary becomes `open/none` or `boundary/terrain`. Move laterally across the stair approach and confirm the route does not flip between the left-ray and right-ray interpretation: `route_left_angle` and `route_right_angle` may identify different outer rays, but all family members must share those source angles and `route_members`, and the audible edge order and opening must remain stable. Repeat with the sides reversed and with a close, short, or single-station barrier; any positively detected hard barrier or edge must make the route `classification:guided`, remain audible rather than being discarded because it is slight, and produce `audio_scale:0`. Reproduce session `1789160486`, dump 2: rays whose connected path reports a descent must remain terrain rather than becoming legacy crouch cues. A genuine low passage whose connected path reports crouch must remain unchanged. The boundary sequence must occur at most once per sweep and must not replace wall, drop, genuine crouch, or ladder cues. At a fork containing separate rising and descending stairs, hold the view still for several sweeps and confirm the route groups alternate without their sequences overlapping, even when one-route sweeps occur between two-route detections. Any unrelated contour tail should duck clearly beneath the boundary sequence. Inspect each structured terrain sample's `corridor`: paired routes should have at least two `bounded` stations, one-sided routes should have at least one positively located guided side, all grouped rays should report the same `route_group` and `route_total`, and exactly one structured representative should have `runway_played:1` per sweep.

Next approach broad indoor floor gradients and broad outdoor hills from both the approach and while standing on the grade. A shallow grade is eligible for suppression only when at least two stations report lateral continuation on both sides at the five-player-width limit; either `open/none` or `boundary/terrain` is continuation, while only `boundary/barrier` and `boundary/edge` can bound a route. Confirm `classification:broad`, `suppressed:1`, and `audio_scale:0`, including on rolling Crash Site terrain where a lateral height difference must not produce a runway trace. A maximum elevation excursion of at least two player widths must remain fully audible on angles from -45 through 45 degrees. At the peripheral -80 and 80 degree opening sentinels, the same substantial broad route must use `audio_scale:0.65`, remain spatially distinct on the appropriate side, and not be mistaken for a nearer forward route. Every true drop remains fully audible. Test a shallow uncertain elevation change ending at a wall within four player widths: it must be silent, while a bounded short stair and a larger or longer uncertain route remain available. This dead-end heuristic is explicitly provisional; record any useful short route it hides. Test one-sided platforms, geometry seams, doors, moving platforms, and room boundaries: other uncertain results must use `audio_scale:0.65`, never silence. Correlate these decisions with the independent `cane/terrain_width_sensor` transition/midpoint widths. With advanced diagnostics enabled, compare `performance/cane_corridor_window` against cane-off and the prior route; inspect `retries` separately and investigate a corridor maximum over 2 ms, recurring budget stops/unresolved errors, sustained frame regression, or any accessibility share that approaches the existing performance thresholds.
...
For the accessible compass, begin with `Accessibility.CompassMode=1`. Confirm Shift+F4 cycles Speech and sonification to Sonification only to Off and back, with a one-time spoken mode confirmation, while plain F4 changes only the cane. Alt+F4, Shift+Alt+F4, Ctrl+F4, and Shift+Ctrl+F4 must change neither feature. Rotate slowly clockwise and counter-clockwise across each exact engine-world cardinal axis and verify North uses one centered 30 ms click, East two, South three, and West four, at 600 Hz with 65 ms gaps. In mode 1, the correct direction word must be requested at the same event as the first click; in mode 2, crossings must produce no direction speech; in Off, they must produce neither. Hover within six degrees on both sides of an axis to confirm jitter cannot chatter, then move beyond six degrees and cross again to confirm it re-arms. Cross North through the 359/0 wrap in both directions, turn quickly, and verify there are no stale queued directions or partial click patterns. Pause or open a menu during four West clicks and confirm the remaining pattern stops; resume, rotate, and confirm a silent baseline prevents a false crossing. Repeat through stage changes, death/respawn, CamSpy entry/exit, mouse and controller rotation, and a Combat Simulator match. Inspect `compass/command`, `scope`, `cardinal`, `observer_change`, reset, heading, rotation, observer, click-count, and speech-result fields; verify the dedicated lane does not steal cane, scanner, targeting, or HUD audio and produces no sustained frame-time or memory regression.
...
For ladder-overlap regression coverage, session `1788981635` must retain the
DataDyne Research ladder: its accepted probe used one query and a
horizontal-facing normal near `24522,0,0`. Session `1788982433`, captures
1–3, must reject floor or ledge overlaps whose normals are near `0,62600,0`.
Those records must increment `ladder_probe_rejections`, leave
`ladder_probe_hit:0`, and retain an ordinary barrier rather than a ladder cue.
This acceptance rule applies to geometry orientation, not to either stage.

Reproduce DataDyne Investigation session `1788985881`, capture 1, near
`-1177,-1882` in room 16. Sweep 130 found center standing clearance through
90 units, crouched clearance through 210, and a wall at 240. It must now report
`crouch_validation:dead_end` and an ordinary barrier because it never recovered
standing clearance. It must not announce a crouch passage merely because the
recess exceeds two player radii. At a real underpass, verify
`standing_recovery` and `standing_continuation` prove at least two radii before
a later wall; a genuinely long crouch route may instead remain open through
the full query range. Confirm point-clearance queries stay within the existing
per-ray count and time budgets.
...
Build with `Accessibility.KingOfTheHillBeacon=1` and start a one-local-player King of the Hill match. With Hill on Radar enabled, confirm a half-gain 300-to-600 Hz dual sweep with one 800 Hz chirp per second continuously points toward the exact native hill marker, uses rear modulation when behind, and remains available at native radar distances and through intervening walls. Press F3 and confirm the hill also appears once in the ordinary radar snapshot as an objective.

Approach the hill. Once direct line of sight and the configured `Accessibility.MarkerRange` are satisfied, confirm the stronger distance-shaped local base replaces the radar guide without interrupting the one-per-second chirp. Break and restore line of sight, cross the range boundary slowly, walk through the exact center, and approach from multiple elevations and headings. The transition must not duplicate two hill voices, steal any of the four F9–F12 marker slots, or chatter at the range boundary.

Enter an uncontrolled hill and confirm the chirp accelerates to twice per second after the player's team captures it and the native point countdown begins advancing. Leave the native hill room and confirm the cadence returns to once per second even if a teammate retains control. Re-enter, then introduce an opposing team member: while the native countdown is paused by contention, the cadence must return to once per second; it must accelerate again when exclusive control resumes. Confirm the faster cadence stops immediately when control is lost or the hill begins moving. Correlate `inside`, `player_team`, `occupied_team`, `elapsed240`, and `scoring` in `hill_beacon/state` and `summary` records with the visible room colour and countdown.
...
### X-Ray semantic-visibility acceptance

With `Accessibility.XRayScannerAudio=1` and F5 enabled, start an X-Ray switch exercise or mission section and activate the scanner. Each hidden switch rendered through geometry must use the standard 880 Hz interactable chirp after no more than the expected one-frame observation delay. It must not also produce the former generic 700 Hz X-Ray cue. Turn away and back, and cross the native eraser-radius boundary, to verify automatic removal and reacquisition.

Where available, repeat with X-Ray-rendered doors, pickups, hostile characters, and non-hostile characters while their corresponding F6, F8, combat, and F7 scanners are enabled. Each must use only its established category sound and capacity. Disable a category and confirm X-Ray does not override that selection. Ordinary furniture and other semantically unclassified scenery must remain silent. Deactivate or inhibit the scanner and confirm the same props immediately return to normal viewport, room, and line-of-sight behavior without changing sticky scanner selections.
...
In the CI FarSight firing-range exercise, test both functions against targets beyond the ordinary firing-range audio cutoff. Every active, front-facing target rendered inside the FarSight volume and viewport must use the range-target presence sound at the configured scoped-distance gain. The exact native aimed target must produce the lock tone even though the FarSight sight does not draw the ordinary target-indicator widget; facing-away, inactive, destroyed, outside-volume, and outside-viewport targets remain ineligible. Confirm switching back to another firing-range weapon immediately restores the ordinary range-distance policy and target-indicator requirement.
