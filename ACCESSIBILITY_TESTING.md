...
Equip the Sniper Rifle, enter its zoom view, and place one visible hostile around each edge and then near the crosshairs. The selected hostile must chirp for about 100 ms every 160 ms; left/right screen error must reach useful stereo separation relative to the narrowed viewport, and the second half must rise for its selected point above the crosshairs or fall for one below. Outside the expanded target rectangle, logs should show coarse `model_hitbox` guidance without detailed queries. Enter the 12-pixel envelope and verify one target reports `precision_fine_attempted=1`; when a detailed surface validates, `precision_anchor_source=model_geometry` must replace the coarse anchor and guide the crosshairs toward a point from which the independent native lock can be acquired. Centering vertically must restore one uniform normal enemy carrier.
...
Correlate `precision_refinement`, `precision_guidance_select`, `precision_guidance_stop`, detailed `combat_candidate`, and `cue=sniper_precision` records. Query count must never exceed 15 per refinement; repeated `budget_exhausted=1` without a later valid point requires more sampling analysis rather than silently increasing the budget. In an advanced-diagnostics build, compare matched stationary scoped scenes with the crosshairs outside and inside the fine envelope. Use `performance/targeting_window` to calculate query microseconds per rendered frame and per detailed query while comparing render FPS and maximum frame gaps. Flag a sustained FPS loss above five percent, more than one millisecond of average refinement work per rendered frame, or repeated refinement maxima coincident with slow-frame gates. Exercise at least one unusual character model; `native_autoaim_fallback` is acceptable only when no usable model collision anchor was captured.
