...
With accessibility and logging enabled, enter ordinary gameplay and move, turn, use controls, and face a known prop for at least 15 seconds. Press Shift+F2 and confirm the screen reader says `Diagnostic capture 1 saved`. Allow at least one more second of play or exit normally so the buffered log flushes. In `accessibility.log`, verify one matching `incident/capture_begin` and `incident/capture_end`, ordered history samples up to the fixed 60-entry capacity, current player and observer state, objective and raw-requirement records, inventory, beacon state and schedule, nearby active props, and a view-blocker result. The end record must report `duration_us`. Quote both the spoken capture number and the log's session identifier in a bug report.

Repeat while using the CamSpy and confirm observer position, room, direction, nearby-prop range, and blocker ray originate from its current camera rather than Joanna. Repeat in pause/menu, cutscene, death, and stage-transition contexts to verify state is captured or reset without stale props or history. F2 alone, Control+Shift+F2, and Alt+Shift+F2 must not capture. With logging disabled, Shift+F2 must say `Diagnostic logging unavailable` without creating a log or affecting gameplay. Trigger several captures in one session and confirm identifiers increase, the history remains bounded, JSON Lines remain parseable, no game/audio memory grows per sample, and collection duration is acceptable. A new process may restart capture numbering at one; session identifiers disambiguate it.
...
The same background-only LOS policy now applies unconditionally to interactable objects rather than only to objects carrying `OBJFLAG2_INTERACTCHECKLOS`. Proximity and room adjacency are prefilters only. The scanner tests LOS while building each refresh snapshot and again immediately before playback. A recessed terminal may pass through one of five conservative points on the closest model-box face when its setup origin is behind supporting world geometry. If all ordinary probes fail, an on-screen object whose native interaction does not request LOS may retry those five points eight units closer to the camera; every retry remains a background collision test, and render state alone cannot admit it. Detailed `beacon/scan_result` records expose `los_sample=0` for the origin, `1` through `5` for an ordinary surface sample, `6` through `10` for a shallow-embed retry, and `los_queries` for the number of early-exit probes.

Regression-test the DataDyne Investigation bot-activation maintenance terminal identified in session `1785248208` as prop 81, tag `0x02`, `MODEL_TVSCREEN`. Capture 1 showed it on-screen, healthy, active, explicitly interactable, about 191 units away, absent from an active F5 scanner, and blocked only by background LOS; capture 2 showed the corresponding `Start security maintenance cycle` objective become complete after native activation. With F5 active and the terminal visibly exposed, confirm it enters the schedule and that a detailed scan reports shallow-embed `los_sample=6` through `10`. Then test a visible CI laptop or terminal, move behind an opaque wall or closed world-geometry partition while remaining nearby, and wait through at least two refresh intervals; its 880 Hz chirp must stop and the log must report `line_of_sight_blocked`. Pay particular attention to a monitor just behind a thin wall but not rendered on-screen: it must remain excluded despite being within eight units of the wall. Restore the direct path and confirm automatic reacquisition without toggling F5. Repeat the occlusion transition after the target is already scheduled to prove the pre-pulse validation uses the same LOS policy.
