...
The same background-only LOS policy now applies unconditionally to interactable objects rather than only to objects carrying `OBJFLAG2_INTERACTCHECKLOS`. Proximity and room adjacency are prefilters only. The scanner tests LOS while building each refresh snapshot and again immediately before playback. A recessed terminal may pass through one of five conservative points on the closest model-box face when its setup origin is behind supporting world geometry; this remains a background collision test and does not use the renderer's on-screen flag as a substitute. Detailed `beacon/scan_result` records expose `los_sample=0` for the origin, `1` through `5` for a bounded surface sample, and `los_queries` for the number of early-exit probes.

Regression-test the DataDyne Investigation maintenance terminal that was previously logged as prop 82, `MODEL_TVSCREEN`, and rejected as `line_of_sight_blocked` despite native activation. With F5 active and the terminal visibly exposed, confirm it enters the schedule and that a detailed scan reports a bounded surface sample when its origin remains obstructed. Then test a visible CI laptop or terminal, move behind an opaque wall or closed world-geometry partition while remaining nearby, and wait through at least two refresh intervals; its 880 Hz chirp must stop and the log must report `line_of_sight_blocked`. Restore the direct path and confirm automatic reacquisition without toggling F5. Repeat the occlusion transition after the target is already scheduled to prove the pre-pulse validation uses the same LOS policy.
