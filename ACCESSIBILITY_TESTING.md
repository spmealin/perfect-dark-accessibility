...
At the Air Base ladder represented in diagnostic session `1786399911`, capture 1, face and approach the climbable surface from several angles. Every cane ray that collides with native ladder geometry must produce three countable ascending rung chirps at the impact point; adjacent ordinary wall geometry must retain the single wall chirp. Confirm the cue preserves ordinary distance pitch, attenuation, and spatial pan, and that `cane/sweep` reports `state=ladder`, `ladder:1`, the native ladder `geoflags`, and the ladder tone pattern. Repeat with player-only ladder geometry where available and verify unflagged decorative rungs do not claim climbability.
