...
For the current-grade prototype, first approach a ramp from flat ground: its beginning must remain a spatial terrain contour. Stop midway on a steady ramp and face along it. The nine rays must no longer repeat that same grade when their floor samples follow the current plane; openings should become silent and walls beyond the slope should sound at their collision positions. One centered rising or falling 140 ms `cane/grade_context` contour must occur during each sweep's end pause. Walk across a crest or slope boundary and turn during both a Slow and Fast sweep: later ray classification must use the logged start plane in absolute world coordinates, while the end-pause contour must use the refreshed current `surface_point`, normal, and camera direction rather than the stale sweep-start grade. Turn across and then against the grade and confirm the context weakens below the 2.5-percent exit threshold or reverses correctly, without rapid toggling around the 3.5-percent entry threshold. At the crest and bottom, plane deviations must restore spatial transition cues. Repeat on discrete stairs, native step-flagged geometry, a low-headroom ramp, a non-traversable rise, a descending ramp, an uneven hillside, and a large drop; stairs, unsafe transitions, and drops must retain their established spatial semantics. Confirm grabbed-object, CamSpy, and hoverbike profiles produce no walking-grade context. Inspect `surface_point`, `surface_normal`, `surface_ground_delta`, `forward_grade_ratio`, `surface_grade_ratio`, `grade_max_residual`, `grade_continuous`, `grade_safe`, `terrain_suppressed`, and `grade_cue_played` in `cane/sweep`, and verify the two current-floor queries do not produce a measurable sustained frame-time regression.
