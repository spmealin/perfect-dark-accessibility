...
In Mission Select, compare each spoken option with its three rendered difficulty stars. Confirm solo play announces every bright Agent, Special Agent, and Perfect Agent marker through the highest completed difficulty; co-op announces only its independently bright difficulty markers; anti announces no completion suffix because the renderer shows no stars. Include regular and unlocked special missions, plus profiles with no completion, one completion, and all completions. In both Combat Challenges and Completed Challenges, compare each option with its one-to-four-player star row and confirm speech reports exactly the bright player-count markers. Repeat in a 4 MB configuration where only the one- and two-player stars are rendered. Confirm incomplete/dim stars are never described as completed.
