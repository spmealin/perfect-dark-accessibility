...
Equip the Sniper Rifle, enter its zoom view, and place one visible hostile around each edge and then near the crosshairs. The selected hostile must chirp for about 100 ms every 160 ms; left/right screen error must reach useful stereo separation relative to the narrowed viewport, and the second half must rise for its selected collision-region anchor above the crosshairs or fall for one below. Centering vertically must restore one uniform normal enemy carrier. Move across standing, crouched, animated, and partially covered guards, including the distant sniper configuration from diagnostic session `1785715598` capture 1: guidance must converge toward projected head, torso, arm, or lower-body collision centers rather than the potentially empty midpoint of the overall model rectangle. Follow the cue onto exposed geometry and confirm that the independent solid lock begins only when the game's exact raw query accepts the model. With two adjacent enemies, move slowly across their midpoint and confirm the selected identity does not chatter; a meaningfully closer target must take over. Leave zoom, switch weapons, occlude or eliminate the target, and verify precision guidance stops or returns that combat slot to ordinary distance cadence. Merely crossing a projected rectangle or collision anchor must never start the solid alignment tone. Correlate `precision_guidance_select`, `precision_guidance_stop`, `precision_guidance` in `targeting/observation`, `precision_anchor_source`, `precision_anchor_hitpart`, `precision_anchor_node`, `precision_anchor_nodes_examined`, `target_screen`, and `cue=sniper_precision` cadence records. Exercise at least one unusual character model; `native_autoaim_fallback` is acceptable only when no usable model collision anchor was captured.
