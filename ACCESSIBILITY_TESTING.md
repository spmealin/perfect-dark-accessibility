...
Enable distinct combinations of F5, F6, F7, and F8, then complete, abort, restart, and advance between consecutive missions and Combat Simulator matches. Every stage teardown must stop the active chirp and discard all prop identities, result indices, schedules, observer pointers, and timing state while retaining the exact four category selections. Eligible gameplay in the next stage must perform a fresh scan and resume those categories without a toggle earcon. Verify that a category deliberately left off remains off, including F7 when moving into or out of a mission where non-hostile cues are unwanted. Initialization and orderly shutdown must still clear all four selections, and no pointer or prop number from the prior stage may appear in the new schedule.
