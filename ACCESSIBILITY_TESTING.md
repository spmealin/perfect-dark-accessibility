...
At a known crouch-only opening, approach while standing and confirm the blocking ray produces a distinct 110 ms downward crouch cue at the opening. Verify full squat can actually traverse the sampled opening, an ordinary wall retains its wall cue, and an obstacle immediately behind a low opening prevents a false passable result. Repeat while ducking, fully squatted, beside rather than facing the opening, and in CamSpy. The cue must not mutate stance or collision state, and CamSpy or an already full-squat collision envelope must not claim crouch semantics. Correlate `state=crouch`, `crouch`, `crouch_result`, `crouch_pass`, and `crouch_ymax` with the standing and squat-height collision outcomes. Confirm ordinary distance still controls every cue's midpoint pitch and attenuation. Record false positives from floor seams, decorative geometry, stacked rooms, lifts, and nearby lower floors.
