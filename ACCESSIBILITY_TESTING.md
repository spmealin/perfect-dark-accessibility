...
In Area 51: Rescue, approach the intact wall marked with the silver X while F5 is enabled. The tagged `OBJ_WALL` prop must produce the ordinary positioned interactable chirp only when the intact marker is on-screen, within the normal object range, room-related, and reachable by the scanner's visual line-of-sight test. Turn away, move out of range, occlude it, and destroy the wall; the cue must stop in every case. This is an explicit mission-landmark exception to native interaction eligibility, not a general rule for non-activatable scenery, and it must not create a weapon lock tone or imply that using the wall is the required action.
